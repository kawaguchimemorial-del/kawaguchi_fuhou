...
export function ScriptStartForm() {
  ...
  function pickEstimate(e: Est) {
    setEstimate(e);
    if (e.deceased) setDeceased(e.deceased);
  }
  function start() {
    ...
    router.push(`/funeral-script?${params.toString()}`);
  }
  const inp = "w-full rounded-lg border border-gray-300 px-3 py-2.5 text-base focus:border-slate-700 focus:outline-none";
  const canStart = !!customer || !!deceased.trim();

  return (
    <div className="mx-auto max-w-xl space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-800">司会台本・会葬礼状を作成</h1>
        <Link href="/kanri/funeral-script" className="rounded border px-3 py-1.5 text-sm">← 一覧へ</Link>
      </div>
      <div className="rounded-lg bg-white p-5 shadow-sm">
        <p className="mb-4 text-sm text-gray-500">誰の台本を作成するか登録してください。顧客と対象者を紐づけると、一覧で対応づけできます。</p>
        ...
        <div className="mt-1 flex gap-2">
          ...
          <button type="button" onClick={() => { setOpen(true); setHits([]); setQ(""); }} className="shrink-0 rounded-lg border border-slate-500 px-4 text-sm text-slate-700">選択</button>
        </div>
        ...
        {customer && estimates.length > 0 && (
          <div className="mt-4">
            ...
            <div className="mt-1 divide-y rounded-lg border border-gray-200">
              {estimates.map((e) => (
                <button key={e.id} type="button" onClick={() => pickEstimate(e)} className={"flex w-full items-center justify-between gap-2 px-3 py-2.5 text-left text-sm hover:bg-gray-50 " + (estimate?.id === e.id ? "bg-stone-100" : "")}>
                  ...
                  {estimate?.id === e.id && <span className="shrink-0 text-xs font-bold text-slate-700">選択中</span>}
                </button>
              ))}
            </div>
            <p className="mt-1 text-[11px] text-gray-400">※ 施行を選ぶと対象者名が自動入力され、一覧で葬儀日と対応づきます。</p>
          </div>
        )}
        ...
        <button type="button" onClick={start} disabled={!canStart} className="mt-6 w-full rounded-lg bg-slate-800 py-3 text-base font-bold text-white disabled:opacity-40">
          作成を開始する →
        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-6" onClick={() => setOpen(false)}>
          <div className="mt-14 w-full max-w-lg rounded-lg bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={search} className="shrink-0 rounded-lg bg-slate-700 px-5 text-sm text-white">検索</button>
            </div>
