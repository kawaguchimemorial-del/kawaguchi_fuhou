    ...
    <div className="mx-auto max-w-3xl space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 text-sm">
          ...
          {e && <Link href={`/kanri/estimates/${e.id}`} className="rounded border border-[#9b2fae] px-3 py-1.5 text-[#9b2fae]">見積を開く</Link>}
          <a href={`/kanri/billing/${iv.id}/receipt`} target="_blank" rel="noopener noreferrer" className="rounded border border-[#9b2fae] px-3 py-1.5 text-[#9b2fae]">領収書PDF</a>
        </div>
      </div>
      ...
      <div className="rounded-lg bg-white p-5 shadow-sm">
        <p className="mb-3 font-bold text-[#9b2fae]">入金を記録</p>
        <form action={recordPayment} className="flex flex-wrap items-end gap-3 text-sm">
          ...
          <button className="rounded bg-[#9b2fae] px-4 py-2 text-white">記録する</button>
        </form>
      </div>
      {e && (
        <div className="rounded-lg bg-white p-5 shadow-sm">
          <p className="mb-3 font-bold text-[#9b2fae]">請求明細（見積より）</p>
