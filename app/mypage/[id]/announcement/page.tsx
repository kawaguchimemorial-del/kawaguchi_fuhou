  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm leading-relaxed text-[#6b6b6b]">
        参列者様に{memorial.hasVenue ? "訃報ページ・オンライン式場" : "訃報ページ"}をお知らせする際は、「URLをコピーする」ボタンを押してコピーしていただくか、
        ページを開いて共有ボタンをご利用ください。
      </p>

      <CopyLinkRow label="訃報ページ" url={obituaryUrl} />
      {/* オンライン式場が無い案件では、開いても中身が無いためリンクを出さない */}
      {memorial.hasVenue && <CopyLinkRow label="オンライン式場ページ" url={venueUrl} />}
