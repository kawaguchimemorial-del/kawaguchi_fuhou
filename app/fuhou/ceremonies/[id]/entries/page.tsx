...
// 訃報案内 閲覧一覧。memorial_views(kind=obituary) を ip_hash 単位（同一IP=1）で集計。
export default async function EntriesPage({ params }: Params) {
  const { id } = await params;
  const stats = await getViewStats(id, "obituary");
      ...
      <div className="overflow-x-auto rounded-lg bg-white shadow-sm">
        <table className="w-full min-w-[420px] text-left text-sm">
          <thead className="border-b bg-gray-50 text-xs text-gray-600">
            <tr>
              <th className="px-4 py-3 font-medium">閲覧日時</th>
              <th className="px-4 py-3 font-medium">訪問者（識別子）</th>
            </tr>
          </thead>
          <tbody className="divide-y">
            {stats.entries.length === 0 ? (
              <tr><td colSpan={2} className="px-4 py-10 text-center text-gray-400">訃報案内の閲覧記録はまだありません。</td></tr>
            ) : (
