...
export function CompanyForm({ values }: { values: Record<string, string> }) {
  ...
  const inp = "mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-[#9b2fae] focus:outline-none";
  return (
    <form action={action} className="space-y-4 rounded-lg bg-white p-6 shadow-sm">
      ...
      <button disabled={pending} className="rounded bg-[#9b2fae] px-6 py-2.5 text-sm text-white disabled:opacity-60">{pending ? "保存中…" : "保存する"}</button>
    </form>
  );
}
