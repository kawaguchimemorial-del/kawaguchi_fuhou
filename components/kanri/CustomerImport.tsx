...
import { importCustomers, type KanriResult } from "@/lib/kanri/actions";
...
export function CustomerImport() {
  ...
  const [state, action, pending] = useActionState<KanriResult | null, FormData>(importCustomers, null);
  ...
  return (
    <div className="space-y-4 rounded-lg bg-white p-6 shadow-sm">
        ...
        <div className="max-h-64 overflow-auto rounded border text-xs">
          <table className="w-full text-left">
            <thead className="bg-gray-50">
              <tr>{Object.keys(rows[0]).slice(0, 8).map((h) => <th key={h} className="px-2 py-1 font-medium">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y">
              {rows.slice(0, 20).map((r, i) => (
                <tr key={i}>{Object.keys(rows[0]).slice(0, 8).map((h) => <td key={h} className="px-2 py-1">{r[h]}</td>)}</tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      <form action={action}>
        ...
        <button disabled={pending || rows.length === 0} className="rounded bg-[#1aa39a] px-6 py-2.5 text-sm text-white disabled:opacity-50">
          {pending ? "取り込み中…" : `${rows.length} 件を取り込む`}
        </button>
      </form>
    </div>
  );
