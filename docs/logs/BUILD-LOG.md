...
## 2026-07-12 — 料理判定を旬菜亭/華鳳含め確実化(商品名フォールバック)

- 料理人数UIの発火条件を、種別「料理」で始まる全種別(料理/料理（旬菜亭）/料理（華鳳）)に加え、商品名一致でも判定するよう強化(productIdが付かない追加経路対策)。

---

## 2026-07-12 — 訃報ごとに「表示する供花・供物」を選択可能に

- 背景: 葬儀規模で使う花/使わない花があるため、訃報(訃報+式場含む)ごとに表示商品を選びたい。
- DB: memorials.flower_product_ids jsonb 追加(migration 0035)。null/空=全表示(後方互換)。
- 管理: CeremonyWizard「供花・供物」ステップに商品チェックボックス(供花/供物)を追加。選択はform_state+専用列に保存。new/edit両ページでlistProducts()を渡す。
- 公開: lib/memorial/db.ts に getMemorialFlowerSelection(slug) を追加(RPCを介さずDB直読み)。/m/[slug]/flower で選択IDに絞り込み(未選択or該当0件は全表示にフォールバック)。
- 検証: 管理の供花ステップに「表示する供花・供物を選択」UI表示(HTTP200)。next build成功。

---

## 2026-07-12 — オンライン供花: 支払い方法設定/請求書自動作成/確認メール/署名非表示

- 支払い方法設定(保存不可だった静的UIを機能化): 請求書払い（銀行振込）と当日現地払いの2方式(カードは非対応)に統合。funeral_homes.order_settings に保存/読込(getOrderSettings/saveOrderSettings)。
- 供花注文フォーム: お支払い方法(請求書払い/現地払い)ラジオを追加。設定で無効化した方式は非表示。
- 注文時に社内管理用の請求書(fk_invoices)を自動作成(lib/kanri/flower-invoice.ts): 顧客は訃報→見積→顧客で解決、請求先種別「オンライン供花注文」・請求先=注文者、明細1行(供花商品・税込→税抜逆算)。
- 見積/請求の宛名種別selectに「オンライン供花注文」を追加。
- 注文確認メールを注文者へ送信(PDFなし・Resend)。
...
## 2026-07-12 — 供花注文に確認ステップを追加(入力→確認→確定)

- 従来は「確認」ボタンで即注文成立していたのを2段階化。
  - 入力→「入力内容を確認する」(クライアント検証・値スナップショット、注文はまだ成立させない)→確認画面(内容一覧)。
  - 確認画面で「この内容で注文する」→submitOrder実行(注文成立・請求書自動作成・確認メール送信〔請求書払いは請求書印刷リンク入り〕)。「修正する」で入力へ戻る(値保持)。
- 入力欄はDOM保持(CSS display切替)で戻っても値が消えない。クライアント検証はサーバのorderSchemaと同条件。
- next build成功。

---

## 2026-07-12 — オンライン供花: 請求先情報の分割/件名/注文一覧の支払方法を修正

### 原因
- 供花注文フォームが「姓のみ＋単一住所」だったため、請求書の名が空・都道府県空・市区町村に住所全部が入っていた。
- 件名が故人名なし。注文一覧の支払い方法が "オンラインカード決済" 固定表示(列未保存)。

### 対応
- 供花注文フォーム: 氏名を 姓/名(+セイ/メイ)に分割、住所を 郵便番号(住所検索付)/都道府県(select)/市区町村/番地/建物 に構造化。確認画面・クライアント検証も更新。
- orderSchema/submitOrder: 構造化フィールドを受け、fullName/fullAddressを合成。offering_ordersへ payment_method を保存(migration 0036)。
...
## 2026-07-12 — オンライン供花: 注文メールを拡充(注文者確認＋葬儀社通知＋送信専用注記)

- 注文者への確認メール: 注文内容＋(請求書払いは)請求書印刷リンク＋「送信専用。お問い合わせはお電話で」注記＋署名。
...
## 2026-07-12 — 供花注文完了画面に迷惑メール確認の案内を追加

- 注文完了画面に「確認メールを送信。届かない場合は迷惑メールフォルダを確認、それでも無ければ電話問い合わせ」の案内を追加。

---

## 2026-07-12 — 供花注文 確認画面に「注文確定」の案内を追加

- 確認画面に「『この内容で注文する』を押すと注文が確定します。修正は『修正する』」の注記を追加。

---

## 2026-07-13 — 見積→訃報作成の誤マッチ(喪主が別人/一覧に出ない)を修正(6専門家)

### 根本原因(6専門家診断)
- findMemorialSlugByDeceasedName のマッチ条件が「没日一致 OR 喪主一致」で、喪主比較は「喪主」接頭辞を除去しておらず常にfalse(死んだコード)。実質「故人名＋没日」だけで一致。
- テストデータで「山田太郎・没日07-11」の訃報が乱立 → 松澤了の見積(喪主/宛名NULL)から訃報作成時、無関係な既存訃報(喪主 山口太郎)に没日だけで一致し、その estimate_id を横取り補完。以後 findMemorialSlugByEstimate が毎回その編集画面へリダイレクトし、新規訃報が作られず・喪主も顧客名にならなかった。

### 修正
- `lib/admin/actions.ts findMemorialSlugByDeceasedName`: (a)他見積にリンク済み訃報は横取り禁止、(b)自見積リンクは本人確定、(c)未リンク候補は「没日 AND 喪主名(接頭辞除去して部分一致)」の両方一致を必須(OR撤廃)。
- 顧客→喪主フォールバックを保存時に追加(`fillMournerFromCustomer`): 喪主未入力かつ見積ありなら見積の顧客名を喪主に採用。createCeremony/updateCeremony両方で発火(新規/編集/再保存を担保)。
...
## 2026-07-13 — kanri UI/UX刷新 v2(15専門家×相互批判会議)

### 進め方
- 15専門家(ブランド/SaaS UI/モバイル/タブレット/a11y/IA/FE実装/CSS/ダッシュボード/テーブル/フォーム/色/タイポ/変更管理/現場代表)が提案→相互批判(各自他14人へ最低5件の否定)→議長統合の3ラウンド。
- 却下された案: 中央FAB / 同席モードのCSS秘匿 / 金の機能色化 / slate青背景 / 13px テーブル / 全体フォント一括上書き / 768-1279全帯アイコンレール 等。

### 実装(3コミット構成・各々独立revert可)
1. retheme(theme-v2.css+layout 2行): [data-kanri-v2]スコープの深緑トークン(#1f6b54系)+ハードコード43クラスの写像(第1層!important)+テーブル/フォーム質感(第2層)。/admin・印刷route無影響。
2. shell: ヘッダー=深緑ダーク帯+金ヘアライン。サイドバー=md未満非表示/md〜lgアイコンレール(タップでオーバーレイ)/lg+フル(折りたたみ・localStorage)。スマホ=下部タブ(ホーム/顧客/見積請求/予定/メニュー)+全メニュードロワー。CRM_NAV単一ソース維持=機能削減ゼロ。
3. dashboard: 今日・明日の予定→要対応(未入金)→クイックアクション(全タイル維持)→KPI→カレンダー→月別推移→新規顧客。

### ロールバック(ユーザー指示があれば即実行)
- タグ `kanri-ui-v1`(5b98484f) をGitHubに保存済み。
...
## 2026-07-18 — 修正: オンライン祭壇の白い額縁が透過して消えていた

### 症状
オンライン式場で額縁を「白」「白花」に変更すると、枠の白い部分が透過して背景が透けていた。元素材 `public/tmp/オンライン祭壇/額縁/png/` は正常。

### 原因
`scripts/clean-altar-assets.mjs` の背景判定が「ほぼ白・低彩度なら背景」(`mn>=232 && mx-mn<=14`)。市松柄(偽透過)を消す意図だったが、白い額縁そのものが同条件に合致するため、外周からのフラッド塗りが枠を突き抜けて塗りつぶしていた。木目の濃い筋だけが残っていた（白の不透明率19.6%＝本来の半分）。

### 修正
色ではなく市松柄の分布構造で判定するよう変更。
- 外周リングから市松の2色を推定（明=254付近／暗=237〜245でファイルごとに異なる）。
- 各画素を明/暗/その他に分類し、積分画像でO(N)の窓集計。**窓内に明暗2色が両方18%以上ある**ものを市松と判定。白枠は明色(254付近)が皆無なので必ず不成立になる。これが決め手。
- 窓判定は柄と枠の境目で不成立になり柄が残るため、確定領域から2色一致画素へ膨張させて縁を詰める2段階構成に。
- ブルー.pngは元から本物の透過(73%)だったため、キーイングせずトリムのみに分岐。

### 検証
不透明率 白 19.6%→39.0%、白花 18.5%→31.5%（枠が復元）。既に正常だった黒/黒リボン/ピンク/ブルーは±2%以内で変化なし。天板の木目は上隅の欠けが直り79.7%→86.0%と改善。実ページ `/m/{slug}/venue/hall` で全額縁をDOM差し替えして描画確認（本番データは変更せず）。修正前後の比較で白枠が可視化されたことを確認。

### 残り
天板の木目の下端にわずかな欠けが残る（旧版より大幅改善のため許容）。

---

## 2026-07-18 — 修正: 喪主アカウント発行後にマイページへ飛べない

### 症状
喪主アカウント発行後、ログインIDは表示されるがマイページへのリンクが無く、葬儀社側から動作確認に行けなかった。

### 修正
- 発行直後パネル: ログインURLがただのテキストだったため「マイページを開く ↗」ボタンに変更。
- 葬儀詳細「喪主／故人」セクション: ログインID行にマイページへのリンクを併記。
- 「喪主 電話番号／メール」が見積・顧客からの推測値のみを見ており、発行時に実際に入力した連絡先が反映されず「—」のままだった。`getMournerAccount` に `phone`/`email` を追加し、発行時の実値を優先表示するよう変更。

### 検証
本番で実アカウント(bqft4cemr4)のログインを確認。マイページに正常遷移し「喪主 松澤 覚 様向けページ」が表示された（アカウント自体は正常で、欠けていたのは導線のみ）。`next build` exit 0。

---

## 2026-07-18 — 改善: マイページのログインIDを事前入力

### 目的
喪主が10桁の英数字を手入力する負担をなくす。＠葬儀も `/account/sign-in?email=xxx@at-sougi.com` とURLにIDを載せている。

### 実装
- `/mypage/sign-in?id=<ログインID>` でログインID欄を事前入力。IDが入っている場合はパスワード欄に autoFocus し、そのまま打ち始められるようにした。
- 値は `trim → 小文字化 → @以降を切り捨て → 32文字まで` に正規化。`BQFT4CEMR4@example.com` のような表記でも `bqft4cemr4` に寄る（＠葬儀のメール形式URLからの流入を許容）。
- 葬儀社側の「マイページを開く」リンク3箇所（発行直後パネル・発行済みバッジ・葬儀詳細のログインID行）に `?id=` を付与。

### 検証
IDあり→欄に入りフォーカスはpassword／IDなし→空／大文字+@付き→正規化される／`"><script>` を渡してもinputのvalue文字列として扱われるだけで実行されない／事前入力状態でパスワードのみ打鍵しログイン成功、を確認。`next build` exit 0。

### 補足
ログインIDはURLに載るがパスワードは別途必要。＠葬儀と同じ設計。

---

## 2026-07-18 — 変更: 喪主マイページから香典表示を撤去

### 背景
自社版では香典機能を提供しないため、＠葬儀に倣って実装した香典関連の表示が不要（かつ喪主に誤解を与える）。

### 削除したもの（すべて喪主マイページ内）
- 芳名録トップの香典精算テーブル（対象年月3ヶ月・合計・手数料5%・振込金額・振込予定日と注記2本）
- 芳名録一覧の「お香典 N円」バッジ
- 参列者詳細の「お香典」行
- CSV出力の `香典金額` `決済ステータス` 列、TXT出力の該当2行
- メール通知設定の「香典決済の通知」チェックボックス
- `lib/mourner/data.ts` の `getKodenSettlement` / `getKodenByDonor` / `Attendee.kodenAmount` / `Attendee.kodenStatus`

### 触っていないもの
公開側の香典ページ `app/m/[slug]/koden`、葬儀社側の「香典決済 利用する/しない」設定、`koden_payments` テーブル、`memorials.mourner_notify_koden` 列は今回の依頼範囲外のため現状維持。通知設定の保存処理は `mourner_notify_koden` を更新しないようにした（既存値を保持）。
...
## 2026-07-19 — 変更: 喪主マイページから4項目を削除

### 削除
不要との判断により、メニューと実装を撤去。
- メール通知設定（`mail-settings` ページ、`MailSettingsForm.tsx`、`saveNotifyAction`、`MournerMemorial` の notifyEmail/notifyReceipt/notifyKoden、SELECT句の mourner_notify_* 列参照）
- お問い合わせ（`contact`）
- 利用規約（`term`）
- 特定商取引法に基づく表記（`transactions`）

あわせて未使用になった lucide アイコン(Mail/Send/FileText/Scale)のimportを整理。芳名録カードの説明文に香典撤去後も「お香典」が残っていたため「参列者の一覧・メッセージはこちらからご確認いただけます。」に修正。

DBの `mourner_notify_email` / `mourner_notify_receipt` / `mourner_notify_koden` 列は残置（列のDROPは破壊的で、再開時に再利用できるため）。

### 残るメニュー（7項目＋ログアウト）
参列者へのご案内 / オンライン式場 / 芳名録 / 入場記録 / 葬儀の写真 / アルバム / アカウント情報 / ログアウト

### 検証
`tsc --noEmit` エラー0 ／ `next build` exit 0（ビルド出力から4ルートが消えたことを確認）。実データでログインし、残す8項目が全て表示・削除した4項目が非表示・旧URL4本が全て404 を確認。

※ 検証スクリプトの `waitForURL("**/mypage/*")` は `/mypage/sign-in` にも一致するため待機にならず、一度サインイン画面を誤って計測した。パス正規表現で判定するよう修正済み。

---

## 2026-07-19 — オンライン式場: 焼香後の文言を削除し、受付導線を1本に

### 変更
- `components/guest/AltarView.tsx`: お焼香ボタン押下後に出ていた「心を込めてお焼香をするいたしました。」を削除。ボタン名(worshipButtonLabel)をそのまま文中に埋めていたため「〜をするいたしました」と二重述語になっていた。押下したことは煙の演出で伝わるため文言自体を廃止。`smoking` state は煙のアニメーションで引き続き使用するため残置。
- `app/m/[slug]/venue/hall/page.tsx`: ご記帳・メッセージ受付セクションから「頂いたご記帳の一覧　›」ボタンを削除し、「受付はこちら　›」のみに。

### 検証
実ページで焼香ボタンを押下し、「いたしました」が表示されないこと・煙要素(7個)が動作すること・受付セクションのリンクが「受付はこちら」1件のみであることを確認。`tsc --noEmit` エラー0 ／ `next build` exit 0。

※ 一覧ページ `/m/[slug]/messages` 自体は残置（訃報トップからの導線があるため）。オンライン式場からの導線のみ削除。

---

## 2026-07-19 — 見積書: 印刷ボタンの重なり解消とメール送信の導線追加

### 1. レイアウト
見積詳細のヘッダーは `justify-between` で件名とボタン7個を同じ行に置いていたため、件名（お客様名を含む）が長いとボタンと衝突して読めなかった。件名を独立した行にし、ボタンは次の行へ（`space-y-3`＋`break-words`）。1400px幅で重なりが解消することを確認。

### 2. メール送信
**機能自体は既に実装済みだった**（`app/kanri/estimates/[id]/send-mail/route.ts` ＋ 印刷画面の `#mailBtn`／html2canvas+jsPDFでPDF化して添付）。ただし見積書PDF画面の中にしか入口がなく、詳細画面から辿れないため気づかれていなかった。

- 見積詳細に「メールで送信」ボタンを追加。`print?mail=1` を新規タブで開く。
- 印刷ルートに `?mail=1` を追加し、`window.load` 後に自動で `sendMail()` を実行（画像読込待ちのため300ms遅延）。
- 宛先未登録の場合は詳細画面の時点で無効表示（「メールで送信（宛先未登録）」＋理由をtitleに）。自動送信側でも二重にガードし、無効ならalertで理由を出す。

### 検証
SMTP(sv15016.xserver.jp:465)へ `transporter.verify()` で接続・認証OK（**実送信はしていない**）。実データで、メール登録ありの見積＝有効リンク＋宛先をtitleに表示／未登録＝無効表示／`?mail=1`＝送信処理が自動起動（send-mailをPlaywrightで遮断し実送信を防止して確認）。`tsc --noEmit` エラー0 ／ `next build` exit 0。

### 運用上の注意
顧客1066件中、メールアドレス登録は**18件のみ**。現状メール送信できる見積は21件しかない。実運用するには顧客情報へのメール登録を進める必要がある。

---

## 2026-07-19 — 見積書・請求書PDF: A4縦で正しく印刷でき、改ページで行が切れないように

### 症状
メール添付のPDFは項目こそ正しいが、顧客がA4縦で印刷すると崩れる。ページをまたぐと行が途中で切れる。

### 原因（旧実装）
```js
html2canvas(document.body,{windowWidth:document.body.scrollWidth})
pdf.addImage(img,'JPEG',0,0,210,ih)          // 余白ゼロで全面配置
while(rem>0){ pdf.addImage(img,'JPEG',0,-(ph*page),...) }  // 297mm等間隔で切る
```
1. 撮影幅が「その時のブラウザ幅」だったため、紙に載せる際の縮尺が毎回変わる。
2. 余白ゼロ。家庭用・コンビニのプリンタは用紙端に印字できない領域があり、端が欠ける。
3. 297mm等間隔で切るので表の行が上下に割れる。

### 修正
`lib/kanri/pdf-script.ts` を新設し、見積書・請求書の印刷ルート双方から使う（従来は各ルートに同じコードが重複）。
- **A4本文幅で組んでから撮影**：撮影前に body 幅を `(210-24)mm` 相当の px に固定し、レイアウト確定後に html2canvas を実行。画面幅に依存しなくなる。
- **12mmの余白**：`@page` と同じ余白の内側に、本文幅186mmぴったりで配置。
- **行を割らない改ページ**：`tr`/見出し/表などの下端を「切ってよい位置」として集め、1ページに収まる範囲で最も後ろの位置を選んで切る。さらに `thead` 内は候補から除外し、見出し行だけがページ末尾に取り残されないようにした。
- 送信前に仕上がりを確認できるよう「PDFを保存」ボタンを両画面に追加。

### 検証
54明細の見積(225d3fa0)で生成したPDFを解析：3ページとも MediaBox が 210.0×297.0mm、画像幅186.0mm・左余白12.0mm。ページ高さは269.1/261.7/221.7mmと不均等＝行境界で切れている証拠。各ページを画像化して目視し、1→2は【ここまでセットに含まれる】直後、2→3は内訳表の直後で切れ、行の分断なし。請求書(b9a5fc87)も1ページ・210×297mm・余白12mmで正常、JSエラーなし。`tsc --noEmit` エラー0 ／ `next build` exit 0。

---

## 2026-07-19 — git push の恒久対策: HTTPS(GCM) から SSH へ切替

### 症状
Claude 側からの `git push` が3分ハングして失敗することがあった。ユーザーが手動で push すると通り、しばらくは Claude からも通る、を繰り返していた。

### 原因
`credential.helper = manager`（Git Credential Manager）を使用。トークン期限切れ時にGCMが対話サインインを求めるが、非対話シェルでは画面を出せず応答待ちのまま停止していた。`ls-remote`（読み取り）は既存キャッシュで通るため、通信障害と紛らわしかった。

### 対処
既に存在した `~/.ssh/id_ed25519` をGitHubに登録してもらい、リモートを SSH へ切替。
```
git remote set-url origin git@github.com:kawaguchimemorial-del/kawaguchi_fuhou.git
```
SSH鍵は有効期限が無く対話も不要なため、期限切れによるハングが起きない。

### 検証
`ssh -T git@github.com` → `Hi kawaguchimemorial-del!` で認証成功。SSH経由の `ls-remote` / `push --dry-run` / 実push が即座に完了することを確認。

---

## 2026-07-19 — 見積もり新規作成: 没年月日の既定値・時間外搬送カード・下代の必須解除

### 変更（すべて `components/kanri/EstimateCreateForm.tsx`）
1. **没年月日の既定値＝本日**。ほとんどの施行は当日〜前日の逝去のため。サーバー(UTC)とブラウザ(JST)で日付がずれてhydration不整合になるのを避けるため、useState の初期値ではなくマウント後の useEffect で設定する。**編集時(`initial.id` あり)と入力済みの場合は触らない**（既存データを今日の日付で上書きしないため）。
2. **新規見積もりに「時間外搬送」カードを1枚あらかじめ表示**（数量1・単価20,000／税込22,000円）。夜間搬送の割増を取り忘れる事故が多いため既定で出し、日中の施行なら削除して使う運用。`opts` の初期化で、復元明細が空 かつ 請求書でない かつ 新規 のときだけ挿入。位置は「オプション」カード内＝「その他オプション、お供えにかかる費用」の直前。
3. **オプションの「下代」から必須バッジを削除**。数量・消費税率の必須表示は従来どおり。

### 検証
新規作成画面で 没年月日=2026-07-19（本日）と一致／「時間外搬送」カードが 数量1・単価20000・税込22,000円で表示／下代ラベルが「下代」のみ／hydration警告・JSエラーなし。
回帰確認: 既存見積の編集＝カード追加なし・没年月日は空のまま維持、請求書の新規＝カード追加なし、お客様入力(intake)画面＝没年月日に本日が入る。`tsc --noEmit` エラー0 ／ `next build` exit 0。

---

## 2026-07-19 — 見積もり・請求書: 数量をダイヤル式＋直接入力の両対応に

### 変更
`components/kanri/EstimateCreateForm.tsx` に `QtyInput` を新設し、オプションカードの数量に適用（このフォームは見積もり・請求書の双方で使われるため、1箇所の変更で両方に効く）。お供え欄も同じ部品に統一した（`min=0`。0個＝付けない があり得るため）。

- −/＋ボタン（h-11 w-11。タブレットで押しやすい大きさ）と、中央の直接入力を併用。
- **入力中は打った文字をそのまま保持する（draft）**。従来の `Number(e.target.value) || 1` は、消した瞬間に1へ跳ね返るため直接入力が事実上できなかった。draft方式にしてフォーカス中は空欄のままにでき、blurで確定する。
- 下限は `min`（オプション=1／お供え=0）。0以下やblur時の空欄は下限に丸める。

### 検証
新規見積もり画面で: 初期1 → ＋2回で3 → −1回で2／全選択して350を直接入力→350／全消しで空欄のまま跳ね返らない→続けて12入力→blurで12確定／0を入れてblurで1に丸め。金額連動も確認（数量1=22,000円・合計92,180円 → 数量3=66,000円・合計136,180円 → ＋で4=88,000円・合計158,180円）。PC(1300px)・タブレット(820px)幅の表示を目視確認。JSエラーなし。`tsc --noEmit` エラー0 ／ `next build` exit 0。

### 補足
`components/kanri/EstimateForm.tsx` は app 配下のどこからも参照されていない旧コードのため対象外。

---

## 2026-07-19 — 削除: 見積もりの「葬儀・法要等」と設定の「法要マスター」

### 削除
- `components/kanri/EstimateCreateForm.tsx`: 「葬儀・法要等」カード（追加ボタンとマスタ一覧チップ）を撤去。`memorialServices` prop も廃止。
- 呼び出し側6ファイルから `memorialServices` の受け渡しと `listMasterItems("memorial_service")` の取得を削除（見積もりnew/edit・請求書new/edit・intake・EstimateCreateWithIntake）。不要なDB問い合わせも1件ずつ減る。
- `lib/kanri/master-defs.ts`: `memorial_service`（法要）の定義を削除。汎用マスタ画面から消えるため `/kanri/settings/memorial_service` は404になる。
- `lib/kanri/nav.ts`: サイドナビの「法要設定」を削除。

### 触っていないもの
`lib/funeral-script/` の `memorial_service` は司会台本の「初七日併修案内」を指す別機能のため対象外（5箇所そのまま）。`fk_master_items` に残る法要データも削除していない（画面から到達できなくなるだけ）。

### 検証
見積もり新規・請求書新規のどちらにも「葬儀・法要等」が表示されないこと、設定ナビに法要設定リンクが無いこと、`/kanri/settings/memorial_service` が404を返すことを実機で確認。セクションは「オプション」→「その他オプション、お供えにかかる費用」→「値引商品」→「前受金」と繋がる。時間外搬送カード・数量ステッパーは維持。`tsc --noEmit` エラー0 ／ `next build` exit 0 ／ JSエラーなし。

※ 検証中に「時間外搬送が消えた」と一度誤検知した。`input` の value は `document.body.innerText` に含まれないため。value を直接見る方法で存在を確認済み。

---

## 2026-07-19 — 見積もり作成の4点修正（お供え既定値／メール保持／ステータス／任意入力）

### 1. お供えの既定値が0になっていた
`initOsonaeQty` の分岐が `if (initial)` だったため、お客様入力(intake)から来た**新規作成**でも「編集」と判定され、既定値(追加安置日数・追加ドライアイス・収骨容器一式・本尊セット一式＝各1)が入らず全て0になっていた。判定を `initial?.id` に変更（時間外搬送カードと同じ基準に統一）。

### 2. 「顧客を同時に新規登録」のチェックで入力が消える
顧客欄の入力のうち**メールアドレスだけが `defaultValue`（非制御）**で、チェックを外すと入力欄ごとアンマウントされ、入れ直すと値が失われていた。他項目は state 管理済みで残っていた。`ncEmail` を追加して制御化。

### 3. メールアドレス（任意入力）
ラベルを「メールアドレス（任意）」にし、「見積書・請求書をメールで送る場合に必要です。」の補足を追加。intake→見積もり画面への持ち越しも `ncEmail` 経由に変更。

### 4. 顧客ステータス
顧客欄に「ステータス」セレクトを追加（`CUSTOMER_STATUSES`、既定=問い合わせ、受注へ切替可）。従来 `resolveCustomerId` は status を渡しておらずDBがNULLだったため、`status: s(fd,"new_customer_status") ?? "問い合わせ"` を保存するようにした。intake からの持ち越しにも対応（`newCustomerStatus`）。

### 検証
intake→入力完了の流れで お供え=1,1,1,0(寝台車),1 を確認。顧客チェックを外して入れ直しても 氏/メール/ステータス が保持されること、対象者など他項目も消えないことを確認。必須項目を全て埋めて実際に登録し、`fk_customers` に `status="受注"` `email="juchu-test@example.com"` が保存されることをDBで確認（検証用に作成した見積1件・顧客1件は削除済み）。`tsc --noEmit` エラー0 ／ `next build` exit 0 ／ JSエラーなし。

## 2026-07-20 管理トップ クイックアクション整理
`app/kanri/page.tsx` の `GROUPS` を修正。
- 「事前相談」と「葬儀発生」の並び順を入れ替え（葬儀発生を先に）。ボタン項目もグループごと丸ごと移動。
- 「葬儀後アフター」グループ（アフターtodo）を削除。
- 「その他」から「営業日報」「顧客登録推移」を削除（入金管理・ユーザー管理は維持）。
- 未使用となった lucide アイコン `CheckSquare` / `TrendingUp` の import を削除。
`tsc --noEmit` エラー0。

## 2026-07-20 よりそうクラウド見積の取り込み
よりそうクラウド（別の見積ソフト／組織ID98・株式会社川口典礼）から施行中葬儀15件を移植。
- 取得手段: Playwrightで1回ログイン→公式「葬儀情報の一括出力」ボタンでCSV(82カラム)をDL。
  ※ api.yoriso.com直叩きはAWS WAFが403で拒否したため、正規のエクスポート機能を使用。
- 取り込み先: `fk_estimates` に15件、`fk_customers`(喪主)に7件を新規作成。件名末尾に「（よりそうクラウド）」を付与。inflow="よりそうクラウド"、estimate_no=よりそう葬儀IDで冪等化。
- ステータス変換: 見積前/一時保存/お迎え通知前→draft、請求前/支払い前/終了→confirmed。
- 金額は一括CSVの「支払合計金額」を total/subtotal に格納（商品明細行はCSV非対応のため未取得）。
- スクリプトはscratchpad内のみ（認証情報を含むためリポジトリ非追跡）。

## 2026-07-20 よりそう 帳票（見積書/請求書）金額の反映
明細行は発行済みPDF内にしか存在せず、アプリは再DL/明細JSONを公開していないことを確認（個別APIは404/405、再発行は副作用のため不可）。ユーザー合意のもと「帳票単位の金額」で確定。
- ブラウザ内fetch（window.fetch）でWAFを回避し `/admin/deal/deals/{deal}/ledgers` を全15件取得（直APIは403/401だがアプリ同一オリジンfetchは200）。
- 帳票発行済みは6件中5件を反映（177107/177550/177889/179705/184075）。各見積の tax_total・discount_total・estimate_on を帳票値で更新し、memoに 見積書/請求書番号・発行日・請求額 を追記。
- 明細商品行（単価×数量）は未取得（PDF内のみ）。必要時はPDFを入手すれば pdf-parse で抽出可能な状態。

## 2026-07-21 商品マスター再編（湯灌・処置の統合）
- 種別「湯灌」「処置」を「湯灌・処置」に統合（処置マスターを改名、湯灌マスターを削除）
- 子カテゴリ新設: 統美 / 警察 / エンバー（parent=湯灌・処置）
- 統美: ラストメイク・納棺一式(¥75,000)、同（立合）(¥80,000)、湯灌の儀式(¥180,000)、香送水処置(¥8,000)
- 警察: 警察対応(¥20,000)
- エンバー: エンバーミング一式(¥400,000) ※重複のエンバーミング¥348,000は削除
- フェアリーメイク(¥20,000)を削除
- 本番Supabase実データを直接更新（fk_master_items / fk_products）。コード変更なし。

## 2026-07-21 商品種別の順番設定（見積もり作成の種別リール並び替え）
- 設定 > 商品種別: 各行の「更新」隣に「順番」リール(select)を追加。選んだ番号にその種別を移動。
...
## 2026-07-21 使いどころが分かりにくいマスタに用途説明を表示（削除はしない）
- MasterDefにhintを追加。rough_product / rough_product_osonae / sale_category に用途・現状の使用有無を明記。
...
## 2026-07-27 葬儀管理ソフト(/kanri) UI刷新 — 8専門家会議の統合仕様に基づく第1弾
- 背景: /kanri が参照元SaaSと画面構成・配色・タイトルバーまで酷似し「模倣」と指摘されるリスクがあった。
  業務フロー/URL/フォーム項目/帳票は一切変えない前提で、見た目と情報設計のみ作り直す。
- 8専門家(IA/ビジュアル/A11y/業務オペ/テーブルUX/モバイル/実装/知財)の個別評価→議長裁定→統合仕様。
  仕様は tmp/kanri-spec.json。却下: 設定のサイドバー撤去/下部タブ再編/PC操作列のケバブ化/施行ボード即時実装/URL改名 ほか。
- コンセプト: 「色帯のSaaS」→「墨と罫線の業務帳票」。指紋は色相ではなく“有彩色の濃いクローム＋その下の緑帯”という構造と判断。
- トークン刷新(theme-v2.css): クロームを墨 #22262b(金ヘアライン#b08d3f廃止)、面を和紙生成り #f6f4ef、
  線 #e0ddd6、角丸8→4px、カード影→none+1px罫線(影はオーバーレイのみ)、--k-ink-faint 追加。
  緑#1f6b54は主ボタン・バッジ等の小面積に限定。text-gray-400/blue-500/red-500 の素クラスをAA適合値へ是正。
- 見出し: PageHeader を白地＋左3pxブランド縦罫＋下辺1px罫線に刷新(actions配列/件数ピルに対応、旧action propは後方互換)。
  各一覧が直書きしている `-m-5 ... bg-[#2c8c6f]` の緑帯も、CSSレイヤーで同じ体裁へ一括変換(30ファイルを触らずロールバック容易)。
- ナビ: サイドバーのアクティブを左3pxバー→角丸ピル、親は丸ドットで所在表示、aria-current付与。
  モバイル下部タブは枠数・順序・hrefを維持しアクティブ表現のみ変更、ラベル「見積・請求」→「施行・請求」。
  ドロワーにメニュー絞り込み入力を追加(ラベル部分一致・構造は不変)。
- トップバー: 「起動」→「新規受付」(遷移先/kanri/customers/newは不変、アイコンUserPlus)、検索placeholderを具体化、
  スキップリンクとmain#kanri-mainを追加。
- ダッシュボード: 当日帯(日付/今日の通夜・葬儀/未入金超過/新規受付)を新設。KPI3枚を上部へ繰り上げ数値と単位を分離。
  96px正方タイル→高さ48pxの横長リスト(リンクは1つも削らず)。カレンダーを右カラム固定から全幅へ。
  月別顧客登録数グラフは削除(created_atが一括取込月に寄り実態と異なる数字を描いていたため)。
- ソース内の参照元名(19箇所)と自認コメントを全除去(app/lib/components に「スマート葬儀」表記0件)。
- 副作用の是正: 文字サイズ底上げで表の操作ボタンが縦積みになったため、表内(td)の極小文字は元サイズに戻した。

## 2026-07-27 /kanri UI刷新 第2弾: 検索パネルの折りたたみ・設定マスタの絞り込み
- components/kanri/SearchPanel.tsx を新設。一覧の検索を <details> で折りたたむ。
  - 入力項目の name / GETパラメータ / 並びは完全に不変（絞り込み結果が変わらないことを最優先）。
  - URLに条件が1つでも付いていれば open で自動展開（再検索の手数を増やさない）。
  - 適用中の条件を削除可能なチップで常時可視化＋「条件をすべて解除」。
  - buildChips() が「その条件だけ外したURL」を生成。
  - 適用: /kanri/customers（キーワード欄は折りたたみの外に常時露出）・/kanri/estimates・/kanri/billing。
  - 検索ボタンの絵文字🔍は削除。
- components/kanri/MasterHub.tsx を新設し /kanri/settings/masters に適用。
  名称・説明の部分一致で絞り込めるようにした（リンク先・並び順・グループは不変。表示フィルタのみ）。
  「基本設定」カテゴリを先頭に追加し、会社情報/商品/サービス利用料/項目表示/必須項目/通知設定を同じ体裁に統合。3カラム化。
...
## 2026-07-30 AI遺影: 髪の破綻に対する指示を追加
- 症状(実データで確認): 生成済み遺影を実際にDLして比較したところ、白髪でも毛量のある方(田中和夫様)は自然だが、
  薄毛＋白髪の方(山田太郎様)で「頭頂部がなめらかなドームに塗りつぶされる」「側頭部の白髪が1本ずつでなく霞になる」
  「髪と背景の境界に淡いハローが出て貼り付けたように見える」が発生していた。
- 原因: プロンプト全体で髪に触れているのは各モード共通の「…髪型…をできるだけ保持してください」の一語のみで、
  毛の質感・生え際・境界処理について何も指示していなかった。
  肩紐除去(IEI_PHOTO_REMOVE_STRAP_PROMPT)には独立した指示があるのに、髪には無い非対称な状態だった。
  明るい背景に対し白髪はコントラストが低いため、AIが毛を描くのを諦めて輪郭を整形してしまう。
- 対応: IEI_PHOTO_HAIR_PROMPT を新設し、buildAiPrompt / buildWideMonitorPrompt の両方で常に適用。
  「白髪・細い髪・毛量の少ない部分・地肌が見えている部分も1本ずつの毛の質感を残す」
  「輪郭を塗りつぶさない/毛量を増減しない/頭の形を整形しない/生え際を動かさない」
  「境界に光の輪・白フチ・黒フチ・貼り付けた境目・不自然なぼかしを作らない」を明示。
  服装・体勢より前に置いて優先度を上げた。
...
## 2026-07-30 見積: 割引が0円になる不具合を修正し、単価のマイナス入力に対応
- 不具合: 見積作成で割引額を入れて完了しても反映されず0円になっていた。
  原因は2つ:
  1) EstimateCreateForm: 値引行を `discRows.filter((d) => d.name)` で絞っており、
     値引商品マスタを選ばず金額だけ入れた行が丸ごと捨てられていた（無言で消える）。
  2) computeItems: 通常行の金額を `Math.max(0, price*qty - disc)` で0未満に切り捨てていたため、
     単価にマイナスを入れても0になっていた。
- 対応:
  - 値引行は「名称または金額のいずれかが入っていれば」拾う。名称未入力時は「値引き」を既定名にする。
  - 金額の0切り捨てを廃止し、単価マイナスをそのまま通す。
  - 集計を line_kind ではなく「金額の符号」で振り分け、マイナスになった行は line_kind='discount' として保存。
    これにより単価マイナスの通常行も、見積書・請求書で既存の「値引」セクション（独立した小計付き）に分かれて出る。
  - オプション行の単価に「値引きは -50000 のようにマイナスで入力」の説明を追加。
- 検証: 葬儀一式50万＋オプション6万＋単価-5万＋値引商品2万 で
  小計560,000 / マイナス合計-70,000 / 税49,000 / 合計539,000 となることを確認。

## 2026-07-31 見積: セットの税込1円ずれ / 単価のマイナス入力を修正
- セット税込の1円ずれ: fk_product_sets は税抜(price)と税込(tax_included_price)を両方持つが、
  印刷は税抜から掛け直していたため 189,000円 → 189,001円 のようにずれていた（43件中24件が該当）。
  税抜を切り捨てに直す案は、逆に1円下がるセット(259,000→258,999)が出るため不採用。
  → 明細に price_including_tax（列は既存・未使用だった）を保存し、印刷はこの確定値を優先する方式にした。
     computeItems で保存、フォームはセット行に setEffInc を、税込単価で入力されたオプション行はその値を送る。
- 単価0のときに「-」を打っても消える件: 入力が `Number(v) || 0` で即数値化され "-" が保持できなかった。
  OptRow に unitPriceText（入力中テキスト）を持たせ、表示はテキスト・計算は数値に分離。blurで正規化する。
- ※ 未対応（要判断）: 商品マスタに税込金額が税抜として登録されているものがある。
  例) 収骨容器一式 unit_price=14300（14,300円は税込。税抜なら13,000円）。
  追加安置日数・追加ドライアイスのマスタは10,000円（税抜）で正しく、見積側に11,000円と入力された形跡。

## 2026-07-31 税込まわりの後始末: 既存明細の埋め直し・マスタ修正・税込入力チェック
- 既存見積明細の price_including_tax 埋め直し: 全19,001明細を走査。847件は保存済、対象は4件のみで埋め込み完了
  （メモリアル華想279,001→279,000 / 家族葬(一日)496,001→496,000 ×2 / 直想189,001→189,000）。
  セット名がマスタと一致し、かつ税抜がマスタ価格と同一の行のみ対象（個別価格の行は触らない）。
  差が1円を超えるものは「マスタ自体の不整合」として除外・報告した:
    メモリアル家族葬（二日） 税抜570,980 → ×1.1=628,078 だが登録税込は628,000（78円の不整合）。要判断のため未修正。
- fk_products「収骨容器一式」の登録ミスを修正: unit_price 14,300 → 13,000（14,300円は税込のため）。税込14,300円で表示される。
- 商品登録フォームに「入力した金額は税込」チェックを追加。
  チェック時は入力値を税込として扱い、割り戻した税抜を hidden で送信して保存する（保存値は常に税抜のまま）。
  右側の欄は税込/税抜が入れ替わり、どちらを登録するのか画面上で分かるようにした。

## 2026-07-31 合計計算を「行ごとの税込の積み上げ」に統一 / 酒井様見積とセットマスタを是正
- computeItems の税額算出を変更。従来は Σ(金額×税率) を丸めていたため、
  印刷（行ごとの税込を積む）と一覧（合計に率を掛ける）で1円ずれる余地があった。
  → 行ごとに税込を確定（price_including_tax があればそれ、無ければ符号付き四捨五入）し、
     税額 = Σ税込 − Σ税抜、合計 = Σ税込 とする。印刷と一覧が必ず一致する。
- 酒井　政行　御葬儀(E20260730-7864)の是正: 税込金額が税抜欄に入っていた3項目を修正。
    追加安置日数 11,000→10,000 / 追加ドライアイス 11,000→10,000 / 収骨容器一式 14,300→13,000（いずれも税込は据え置き）
  合計を再計算: 小計662,210→658,910 / 消費税57,130→56,799 / 合計628,431→624,800。
- fk_product_sets「メモリアル家族葬（二日）」の税抜を 570,980 → 570,910 に修正。
  過去79件の見積が570,910で運用されており、登録税込628,000（÷1.1=570,909.09）とも整合する。
  570,980は移植時に混入した誤りで、これを拾った見積は松澤了様(山田太郎様)の1件のみ。

## 2026-07-31 喪主マイページ: オンライン式場が無い案件では式場関連を出さない
- lib/mourner/data.ts の MournerMemorial に hasVenue を追加（memorials.venue が null かどうか）。
  select に venue を追加。訃報のみ(ONLY_OBITUARY相当)の案件は venue=null で判別できる。
- /mypage/[id] のカード出し分け:
  非表示にするもの … オンライン式場 / 芳名録 / 葬儀の写真 / アルバム（いずれも式場の機能）
  残すもの … 参列者へのご案内 / 入場記録 / アカウント情報
  ※ 入場記録は訃報ページの閲覧ログも含むため式場が無くても意味がある。説明文だけ実態に合わせて出し分ける。
- 直リンク対策として online / attendees / funeral-photos / album の各ページに
  「hasVenue が false なら /mypage/[id] へ戻す」ガードを追加。
- 参列者へのご案内: 式場が無い場合はオンライン式場ページのURLコピー行を出さず、説明文も訃報ページのみに変更。
- 対象規模: venue が null の案件は155件（全225件中）。

## 2026-07-31 司会台本: 実マニュアルを精読し、オリジナルナレーションの生成方針を刷新
- 一次資料: F:\onedrive\...\松澤\司会マニュアル\（司会マニュアル/ショートフレーズ集/簡易マニュアル 計66ページ・画像）。
  10専門家で分担精読し、各自が他9名へ反対意見（計90件）→議長が統合。仕様は tmp/narration-spec.json。
- マニュアルの中核（実装に欠けていたもの）:
  ・開式前ナレーションは 導入→本文→結び の3部構成。基礎は「客観的な立場」。
    故人への呼び掛け／踏み込みすぎ／極端に詩的な表現は避け、司会者の独り善がりにしない。
  ・本文の3手法 = 強調(倒置。印象づけたい語を最後へ)／故人の言葉(『』引用・直前に間)／並列(説明を挟まず列挙)。
  ・導入は4型（遺影・音楽・前夜の通夜・生年月日）。遺影が相当古い写真なら由来には触れない。
- prompt.ts の主な変更:
  ・最上位に「絶対に守る三点」を新設（捏造禁止・忌み言葉・役割分担）。埋没していた最重要ルールを引き上げ。
  ・語り口の基本（呼び掛けない/感想を書かない/心情を代弁しない/履歴書順に並べない）を追加。
  ・導入4型と、3手法を「条件付き許可＋回数上限」で追加（義務化すると着地語や添え文を捏造するため）。
  ・闘病等は経緯・評価を書かず、その場面での行いだけを一文で。逝去を強調技法の対象にしない。
  ・分量を素材量連動に（記述が二項目以下なら大きく下回って終える／水増し禁止）。
- ng-words.ts: 提示用 PROMPT_AVOID_WORDS と検出用 NG_WATCH_WORDS を分離。
  「お別れ」「生き続ける」はマニュアル自身が使う正しい言い方なので提示側から除外。
- expressions.ts: extractYear()（和暦対応）と eraExpression() を新設。
  ※ 会議は「生年からの時代描写」を捏造リスクとして却下したが、ユーザー判断で採用。
    実際の司会者は時代背景を入れており、マニュアルの導入例④もその型。
    ただし「時代そのものの描写」は可・「その方個人が戦地/空襲/被災を経験したと書く」のは禁止、と切り分けた。

## 2026-07-31 司会台本 第2弾: 取材項目を追加し、引用『』を発火させる
- 第1弾で入れた「引用は入力にある場合のみ」という条件が、入力欄が無いため常に不発だった。これを解消する。
- types.ts に追加: deceasedWords / familyWish / interviewSource / altarItems / mayMentionIllness。
  すべて任意。既存の保存データは undefined になるため、従来どおりの動作になる。
- prompt.ts に buildInterviewBlock() を新設し「# 取材で伺ったこと（最優先で扱う）」として差し込む。
  ・familyWish があれば「台本の芯はこれに合わせる」と明示（他の素材はこれを支える）。
  ・deceasedWords は「要約も敬語化もせず、伺ったままの口語で『　』に一度だけ引用。直前で改行して間を作る」。
    併せて「ここに書かれていない言葉を『　』に入れることは絶対に禁じる」を同じブロックに置く。
  ・interviewSource は出典に触れるのを台本全体で一度だけに制限。
  ・mayMentionIllness は未チェック(undefined含む)なら「一切触れない」を明示的に指示。安全側に倒す。
  ・altarItems は故人情報の箇条書きへ（会場に実際にあるものだけ、と欄の説明で限定）。
- フォームに「ご遺族への取材」セクションを新設。入力欄の直下に使われ方の説明を添えた
  （敬語に直さずそのまま引用される／台本の芯になる／未チェックなら闘病に触れない）。
- format.ts の空フォーム初期値にも5項目を追加。

## 2026-08-01 司会台本: 一覧に保存されない原因の特定と修正／生成モデルをOpus 5へ
- 原因1(保存): 画面上部の「保存・引き継ぎ」と入力欄下の保存パネルは `handleSaveFile`＝
  JSONを端末にダウンロードするだけで、`/api/funeral-script/save` を呼んでいなかった。
  DB保存を行う `handleSaveToList` はAI生成パネル内の保存にしか繋がっておらず、
  `fk_funeral_scripts` の最新は7/10のテスト行のまま（酒井様の行は存在せず）。
  → 3箇所すべてを `handleSaveToList`（サーバー保存→成功後に端末DL）に統一。ラベルも「保存（一覧へ）」に変更。
- 原因2(精度): ナレーション/会葬礼状の既定モデルが `claude-opus-4-8` のままだった。
  → コード既定を `claude-opus-5` に更新。加えて Vercel の `ANTHROPIC_MODEL`（Production/Preview）も
  `claude-opus-5` に貼り替え（環境変数がコード既定を上書きするため）。
- 付随: `.env.local` の変数名が `ANTHROPIC_API_K` と欠けており、ローカルではAI生成が動作しない状態だった → `ANTHROPIC_API_KEY` に修正。

## 2026-08-01 司会台本: マニュアル『ショートフレーズ集』を実素材としてプロンプトへ組み込み
- 背景: サイト生成の質が手作業に劣る要因として、モデル世代（別項で対応）に加え、
  プロンプトにマニュアルの要約しか入っていなかった点があった。
  `docs/funeral-script/source-extraction/` は存在せず、原文はコード上に無かった。
- 対応: 松澤\司会マニュアル\ショートフレーズ集（全38スキャン）を通読し、
  `lib/funeral-script/phrase-book.ts` に原文を起こした。
  - THEME_EXAMPLES(49件): 職業・趣味・ご家族・遺影/写真・故郷・時代・儀礼の場面
  - SEASONAL_PHRASES_BY_MONTH: 12か月の季節フレーズ（寒晴/初音/朧月/花明り/風薫る/
    風光る/五月雨/夏木立/雲の峰/蝉時雨/夏の果て/山粧う/色なき風/小春日和/哀惜/冬茜/年用意 の語釈つき）
  - GENERAL_CLOSERS / CRAFT_NOTES / WORD_GLOSSARY
- prompt.ts に `buildPhraseBookBlock()` を追加。故人情報の自由記入欄を照合し
  関連テーマ最大4件＋生没月の季節句だけを載せる（全件は載せず肥大を防止）。
  「丸写し禁止／実例中の要素が故人情報に無ければ使わない／学ぶのは運び方だけ」を明示。
- 酒井様のフォームで検証: 自分の事より人の事／頑固一徹好々爺／麻雀／献酒 が選定され、
...
## 2026-08-01 司会台本: 施行(見積)の登録済み情報をフォームへ自動入力
- 背景: 台本作成のたびに生年月日・没日を手入力し直しており、打ち間違いがそのまま
  ナレーションの事実誤りになる。作成導線(/kanri/funeral-script/new)は既に
  customer_id / estimate_id を渡していたが、対象者名しか使っていなかった。
- lib/funeral-script/prefill.ts を追加。見積→フォームの写し取り。
  - 取り込む項目: 対象者名・生年月日・没日・行年・喪主名・続柄・式場・開式時刻・式種別
  - 日付は和暦(昭和30年3月19日)。元号境界は 昭和元年=1926-12-25 / 平成元年=1989-01-08 /
    令和元年=2019-05-01 / 大正元年=1912-07-30 で、元年表記にも対応
  - 時刻は UTC保存→JST変換のうえ読み上げ形(午前11時40分)。12時台は通例どおり「午後0時」
  - 式種別は宗教区分から判定。仏式で通夜・告別式の両方が登録済みなら通夜・告別式の統合台本
- app/api/funeral-script/prefill: estimate_id 指定で取り込む。customer_id のみのときは
  施行が1件だけの場合に限る(複数あると別人の生年月日を読み上げかねないため何も返さない)。
...
## 2026-08-02 ログインをID方式にし、担当者名を自動で入れる
- ご指定により、ログインはメールアドレスではなく短いIDにした。
  `ishikawa` / `matsuura` の2アカウントを発行（パスワードはご指定の固定値）。
- Supabase Auth はメールアドレスで人を識別するため、`lib/admin/login-id.ts` で
  ID＋社内専用ドメイン(`@kawaguchi-tenrei.local`)を内部のメールアドレスとして扱う。
  実在しないドメインなのでメールは送られない。@ を含む入力はそのまま使うため、
  既存の kawaguchi.memorial@gmail.com もこれまでどおりログインできる。
  IDは大文字で打っても通る（小文字化してから照合）。
- ログイン画面の入力欄は「メールアドレス」→「ID」。スマホの自動大文字化・自動補正を切った。
- 担当者の自動入力: `admin_users.display_name` を新規作成フォームの
  「計上担当者」「担当者（葬儀担当）」の初期値にした。
  対象は 見積もり登録 / お客様入力 / 請求書登録 の3画面（新規のみ）。
  編集画面には渡していない（既存データを黙って書き換えないため）。
  選択肢(STAFF_OPTIONS)に無い名前は入れない（selectの値が合わず空欄に見えるため）。
  → 表示名は選択肢と同じ表記にしてある: 石川健太 / 松浦 颯大。
- 検証（本番ビルドをローカル起動しPlaywright、全15項目 ✓）:
  ishikawa/matsuura でログイン → ヘッダーに本人名／見積もり登録・請求書登録の
  計上担当者と担当者が本人名で入っている／他人のパスワードでは入れない／
  失敗してもID欄は残る／大文字IDでも入れる。
  認証まわりの回帰（未ログインの遮断・API401・ログアウト等）も再実行して従来どおり。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。
- ※ 安全面の申し送り: kenta / souta は5文字で、総当たりにはほぼ耐えません。管理画面には
  顧客1,086件・請求1,277件と本番決済の返金機能があります。ご指定どおり設定していますが、
  外部に出す前に長いパスワードへ変更されることをおすすめします
  （`node scripts/manage-admin-user.mjs password ishikawa <新パスワード>`）。

## 2026-08-02 ログインパスワードを12文字へ変更
- ご指定により ishikawa2303 / matsuura2303 に変更（5文字→12文字）。総当たりの心配は実質なくなった。
  パスワードはDBにハッシュで入るためコード・ログには残らない（変更は
...
## 2026-08-02 管理者アカウントのパスワード変更
- kawaguchi.memorial@gmail.com（発行時の自動生成パスワード）を kawaguchi1117 に変更。
  本番で旧パスワードが通らないこと・新パスワードでログインできることを確認（2項目 ✓）。
- ※ このアカウントだけメールアドレスでログインする（ID方式ではない）。表示名は「川口典礼 管理者」で、
  担当者の選択肢(STAFF_OPTIONS)には無いため、見積の計上担当者/担当者は自動入力されず空欄のまま。

## 2026-08-02 AI遺影: 服装を見本写真から選ぶ方式に変更（背景の下へ独立・実際に着せ替え）
- 経緯: 服装は「喪服（和装）／喪服（洋装）／スーツ／カジュアル」の文字ボタンだけで、
  出来上がりが想像できなかった。特にカジュアルは何が出るか分からなかった。
  置き場所も「AI詳細・準備中項目」の折りたたみの中で、気づきにくかった。
- **見本画像を26点生成**（gpt-image-2）。男性13・女性13。
  喪服（和装）／喪服（洋装）／スーツは各1点、カジュアルは各10点。
  見本は「頭部の無いトルソーに着せた服」を白背景・正面で生成（顔を出すと、
  その顔の人になると誤解されるため）。512px webp、1点6〜20KB・合計304KB。
  `scripts/gen-clothing-samples.mjs` で再生成できる（既存はスキップ、id前方一致で作り直し）。
  置き場所は `public/iei-clothing/`。※ `/iei-photo` 配下に置くと middleware の保護対象と
  重なり、画像だけ401→リダイレクトになって next/image が400を返す（実際に踏んで移動した）。
- **UI**: 「AI背景」の直下に独立した「服装」セクションを新設し、AI詳細の中の文字ボタンは廃止。
  男性／女性の切替＋「服装はそのまま（着せ替えない）」＋見本サムネイルの3列グリッド。
  系統ごとに見出しを付けた。選択中は金枠でハイライト。
- **生成**: 選んだ見本画像そのものを2枚目の入力としてOpenAIへ渡す（images/edits の image[]）。
  文字だけの指示より見本を優先し、「見本から使ってよいのは服の種類・色・柄・襟元・素材感だけ。
  トルソー・体型・姿勢・背景は使わない。顔・髪・肌・体格・構図は元のまま」を明示
  （IEI_PHOTO_CLOTHING_REFERENCE_PROMPT）。見本の取得に失敗しても生成は止めず、
  従来の文字指定にフォールバックする。16:9(wide)は左右背景を伸ばす処理なので見本は渡さない。
- 検証（本番ビルドをローカル起動しPlaywright、実データで生成）:
  服装セクションが背景の下に出る／見本画像が実際に読み込まれる(naturalWidth=512)／
  男性13点・女性へ切替で女性の見本に変わる／実写真(tmp/iei-sample-1.png)で
  「喪服（和装）」→黒紋付羽織袴（家紋・羽織紐まで見本どおり）、「作務衣」→紺の作務衣に
  着せ替わり、いずれも顔・髪・体格・構図は元のまま。`npm run lint` 0件・`tsc --noEmit` 0件・
  `next build` 成功。

## 2026-08-02 本番反映（服装の見本選択）
- push 済み(dee68aa)。本番 kawaguchi-fuhou.vercel.app で確認（5項目 ✓）:
  服装セクションが背景の下に出る／見本画像が実際に読み込まれる(naturalWidth=512)／
  男性13点／女性へ切替で女性の見本になる／見本を選択できる。
  ※本番でのAI生成（着せ替えの実行）は課金が発生するため、ローカルの本番ビルドで確認済み。

## 2026-08-02 AI遺影: 髪型が変わる問題を修正／使用モデルの確認
- 症状(実データ): 木村家の実験用写真で着せ替えたところ、顔と服装は良いが髪型が変わっていた。
  元写真は前髪が長めで跳ね、サイドが耳を覆い、もみあげも長い。生成後は短く刈り揃えられ、
  前髪が整えられ耳が出た状態＝散髪した別人の印象になっていた。
- 原因: IEI_PHOTO_HAIR_PROMPT は「毛の質感・生え際・境界処理」（薄毛/白髪対策で7/30に追加）が主で、
  髪型そのもの（長さ・分け目・前髪・もみあげ・襟足・耳が隠れるか）には触れていなかった。
  加えて基本方針の「遺影写真として自然で品のある写真に整えて」が髪にも及び、
  “整える＝散髪してセットする”と解釈されていた。指示の位置も服装・構図の後ろだった。
- 対応:
  - IEI_PHOTO_HAIR_PROMPT を書き換え。「髪型は絶対に変えない（最優先）」を先頭に置き、
    長さ(前髪/サイド/もみあげ/襟足)・分け目の位置と向き・耳が隠れているか・額の見え方・
    ボリューム・外形をそのまま保つことを列挙。切る/短くする/長くする/刈り上げる/セットする/
    櫛で整える/分け目を作り直す/前髪を上げる/耳を出す/隠す/左右対称に整える を名指しで禁止。
    跳ねた毛・寝ぐせ・乱れも残すこと、「整える対象は明るさ・背景・服装だけで髪は含めない」を明記。
  - 指示の位置を、基本方針の直後（最優先）へ移動。以前は服装・構図の後ろだった。
  - 服の見本を使う場合の指示にも「服に合わせて髪を整えることは絶対にしない」を追加。
- 検証: 同じ木村家の写真で2パターン再生成（カジュアル「シャツ」／「喪服（洋装）」）。
  いずれも前髪の跳ね・サイドが耳を覆う長さ・もみあげ・分け目・ボリュームが元写真のまま保たれた。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

### 使用モデルの調査結果（ご質問の回答）
- **遺影の指示文（プロンプト）を書いているのはLLMではない。** `lib/iei-photo/ai-prompts.ts` に
  日本語で直接書いたコード上の固定文で、Claude も GPT も介在しない（＝どちらで生成しているか、
  という問い自体は該当なし）。文言を変えたいときはこのファイルを直すのが唯一の経路。
- 画像の生成のみ OpenAI `images/edits` の **gpt-image-2**（`OPENAI_IMAGE_MODEL`）。
- 文章生成（司会台本のナレーション・会葬礼状）は **Anthropic の claude-opus-5**
  （`lib/funeral-script/ai.ts` → api.anthropic.com、既定 claude-opus-5 / `ANTHROPIC_MODEL`）。
- `.env.local` に `OPENAI_TEXT_MODEL=gpt-5.5` が残っていたが、**コードのどこからも読まれていない**
  （移植時の残骸）。誤解のもとなので無効化（コメントアウト）した。GPTで文章生成している箇所は無い。

## 2026-08-02 本番反映（髪型の指示強化）
- push 済み(4bf7792)。変更はサーバー側のプロンプト文字列のみで、画面上の見た目には現れないため、
  本番での確認は行っていない（確認するにはAI生成の課金が発生する）。
  ローカルの本番ビルドで、実写真2パターンにより髪型が保たれることを確認済み。

## 2026-08-02 AI遺影: 髪の量・厚みが失われる問題を修正（頭頂部が潰れる）
- 症状(実データ・木村家): 前回の対応で髪型（長さ・分け目）は変わらなくなったが、
  頭頂部のボリュームが減り、髪が頭に貼りついて平らになり、跳ねた毛も消えて、
  頭が小さく見える＝別人の印象になっていた。ご指摘のとおり。
- 原因: 前回の指示は「形（長さ・分け目・耳が隠れるか）」を止めることに寄っており、
  「量・厚み・高さ」には触れていなかった。AIは形を守りながら、髪を1つの塊として
  描き直す際に高さと毛束を落としていた。
- 対応: IEI_PHOTO_HAIR_PROMPT の先頭（最優先条件の直後）に、量・厚みの節を新設。
  「頭頂部の髪の高さ・ふくらみ・立ち上がりを元写真とまったく同じだけ残す」
  「頭皮に貼りつかせる／なでつける／押さえつける／濡れたように見せる／平らにする／
  　頭を小さく細く見せる／髪の外形を丸くきれいに整える」を名指しで禁止。
  「毛束のかたまり感・ふくらみ・外へ跳ねた毛・輪郭からはみ出した毛先を1本残らず残す」
  「仕上がりを元写真に重ねたとき、髪を含めた頭の外側の輪郭がほぼ一致していること」
  という判定できる基準も加えた。
- 検証: 同じ写真で2パターン再生成（カジュアル「和装（着物）」／「喪服（洋装）」）。
  縦・16:9の両方で、頭頂部の立ち上がり・跳ねた毛・サイドのボリューム・もみあげが
  元写真のまま保たれることを確認（tmp/hair-compare.png で頭部を並べて比較）。
  ※ 前髪の位置がごくわずかに上がる傾向は残る。これ以上詰めるなら、
  　 頭部を編集禁止領域として mask で守る方式（顔・髪はピクセルごと保存し、
  　 服と背景だけ生成）が次の手。実装には頭部の領域推定が要る。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-02 AI遺影: 髪の個性が「きれいに整えられて」しまう問題を、APIの設定で解決
- 症状(実データ・木村家): 毛量は戻ったが、頭頂部の分け目・立ち上がった毛束・跳ねた毛といった
  その方の髪の個性が均されて、櫛を入れた別人のようになっていた。
- 原因と判断: 文章での指示（「変えないでください」を何通りにも書く）はここが限界だった。
  gpt-image-2 の edits は既定で入力画像の細部を保持せず、人物を"描き直す"ため、
  細い毛の1本単位の情報が残らない。言い方を変えても根本は変わらない。
- 対応: OpenAI edits の **`input_fidelity: "high"`** を常に付けるようにした
  （app/api/iei-photo/ai-image/route.ts）。入力画像の細部（顔・髪の毛流れ）を保ったまま編集する設定。
  縦・16:9の両方に適用。
- 検証: 同じ写真で再生成（カジュアル「セーター」）。頭頂部の立ち上がった毛束、跳ねた毛、
  額にかかる細い毛、分け目の乱れが元写真のまま残ることを確認（tmp/hair-compare2.png）。
  16:9側も同様に細部が残っている（tmp/hair-pw.png）。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。
- ※ コスト: input_fidelity=high は入力画像のトークンが増えるため、1回あたりの生成費用が上がる。
  仕上がりへの影響が大きいので常時オンとしたが、費用が気になる場合は設定で切り替えられるようにする。

## 2026-08-02 AI遺影: 「元写真の細部を残す」を画面から切り替えられるようにした
- input_fidelity=high は仕上がりへの影響が大きい一方、入力画像分のトークンが増えて費用が上がるため、
  常時オンではなく画面で選べるようにした（ご要望）。
- 「AI仕上げ」の生成モードの直下にチェック「元写真の細部を残す（推奨）」を設置。**既定はオン**。
  説明文: 髪の毛流れ・分け目・跳ねた毛など個性を残す／外すと費用は下がるが髪や顔が描き直される。
- 経路: page.tsx の状態 → requestAiImage / requestAiWideImage → `preserveDetail` →
  route が true のときだけ `input_fidelity: "high"` を付ける。縦・16:9の両方に効く。
  API側の既定は true（省略時も残す）。古い呼び出しで黙って仕上がりが落ちないようにするため。
- 検証: 既定でオン／オン時は preserveDetail=true が送られる／チェックを外せる／
  オフ時は false が送られる／オフのまま実際に生成しても正常終了（縦・16:9の2枚）を確認。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-02 本番反映（髪の個性の保持・細部を残す切り替え）
- push 済み(382519b)。本番 kawaguchi-fuhou.vercel.app で確認（3項目 ✓）:
  「元写真の細部を残す（推奨）」のチェックが表示される／既定でオン／生成時に設定が送られる。
  ※実際のAI生成（着せ替え・髪の再現）は課金が発生するため本番では回さず、
  　ローカルの本番ビルドで実写真により確認済み。

## 2026-08-02 【不具合と修正】input_fidelity は gpt-image-2 では使えない（AI生成が全て失敗していた）
- **私の誤りです。** 直前に入れた `input_fidelity: "high"` を gpt-image-2 は受け付けず、
  400 "The model 'gpt-image-2' does not support the 'input_fidelity' parameter." になる。
  この状態を push したため、本番のAI遺影生成は「AI生成リクエストが受け付けられませんでした」で
  すべて失敗していた（502）。
- 検証を誤った経緯: 追加後の確認時、3111番ポートで前のサーバーが動いたままで
  `next start` が EADDRINUSE で起動できず、**変更前の古いサーバーに対して**テストしていた。
  そのため「成功した・髪が良くなった」と誤って報告した。以後、テスト前に必ずポートを空ける。
- 事実関係（実測で確認）:
  - gpt-image-2 に input_fidelity を送ると 400。gpt-image-1 は 200（対応しているのは旧モデル）。
  - gpt-image-2 は指定なしでも入力画像の細部を保って処理する。
  - 髪の個性が戻ったのは **プロンプト側の「量・厚み」の指示（6ce4eb6）の効果**であり、
    input_fidelity は無関係だった。
- 修正: input_fidelity の付与を撤去し、二度と足さないよう理由をコード内に明記。
  併せて「元写真の細部を残す」チェックも撤去（gpt-image-2 に該当する設定が無く、
  切り替えても費用も仕上がりも変わらないため。費用の説明は別途お渡しする概算のとおり）。
- 検証（ポートを空けてから実施）: 画面にチェックが無いこと／実写真で生成が正常終了（縦・16:9の2枚）／
  頭頂部の立ち上がった毛束・跳ねた毛・分け目の乱れが元写真のまま残ることを確認。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-02 AI遺影の生成費用（実測にもとづく概算）
- 単価(OpenAI公式・gpt-image-2): テキスト入力 $5/1M、画像入力 $8/1M、画像出力 $30/1M。1ドル155円で換算。
- トークン数は推測せず、実際のAPI応答の usage を実測した:
  - 縦(1024x1536, quality=high) 入力画像1枚: 入力1,480(画像1,452/文28) 出力5,488 → 約27円
  - 縦・服の見本つき(入力画像2枚): 入力2,505(画像2,476/文29) 出力5,488 → 約29円
  - 16:9(1536x864): 入力1,467 出力4,312 → 約22円（プロンプトが長いので文トークンは400で計算）
- 「背景込みでAI生成」1回 = 縦＋16:9 の2回呼び出し → **約50円**。作り直すたびに同額かかる。
  1件のご遺影で3回試せば約150円。10件で約500円、30件で約1,500円、100件で約5,100円。
- 費用の内訳は出力画像トークンが約8割で、入力（元写真・服の見本）は数円。
  服の見本を足しても+2円程度で、費用面の心配は不要。
- 服の見本26点の生成は一度きりで、概算500円程度（作り直さない限り再発生しない）。

## 2026-08-02 「生成できませんでした」のご報告への回答（原因＝上記の私の不具合／現在は復旧）
- 原因は 2026-08-02 の input_fidelity の不具合。gpt-image-2 が 400 を返し、画面には
  「AI生成リクエストが受け付けられませんでした。別の画像でお試しください。」と出ていた。
  該当していたのは、13:03 のデプロイ〜14:54 の修正デプロイまでの約2時間。
- 現在は復旧済み。本番(kawaguchi-fuhou.vercel.app)で実写真により再確認:
  縦・16:9とも 200 image/png で正常終了（所要 205秒）。
- 併せて分かったこと: 1回の「背景込みでAI生成」は本番で3〜4分かかる（縦+16:9の2回×各100秒前後）。
  画面が固まったように見えるが待てば完了する。体感が悪いので、進捗表示の改善は今後の課題。

## 2026-08-02 AI遺影: 待ち時間の見せ方を改善（経過表示・16:9は後回し）
- 背景: 1回の生成に本番で3〜4分かかる（縦+16:9の2回×各100秒前後）。その間、画面が
  「AI生成中…」のまま動かず、止まったように見えて、実際に途中で操作をやめる要因になっていた。
- 経過時間の表示: メッセージ欄に「経過 1分12秒 ／ 目安 1〜2分」を1秒ごとに更新して出す。
  縦の生成中は「そのままお待ちください（画面を離れると最初からになります）」、
  16:9の作成中は「この間も、できあがった写真はご確認・ダウンロードいただけます」を併記。
- 16:9を後回しに: 縦ができた時点でプレビューへ反映し、状態を「完了（16:9モニター用を作成中）」にして
  操作を解放する。写真の確認・基準写真DL・設定変更・作り直しが、16:9を待たずにできる。
  16:9が出来るまでの体感待ち時間が 3〜4分 → **約1分40秒** になった。
  - 「4サイズDL」だけは16:9を含むため作成中は押せない（ボタンに「16:9作成中…」と表示）。
  - 待っている間に作り直した場合、古い16:9が後から割り込まないよう通し番号で弾く。
  - 16:9だけ失敗した場合は、できた写真を残したまま「16:9モニター用だけ作成できませんでした」と出す
    （以前は全体を失敗扱いにしていた）。
- 検証（ポートを空けてから、実写真で2回）: 生成中に経過が出る／縦が先に出る(99秒)／
  縦の確認・DLができる／設定を触って作り直せる／4サイズDLは16:9作成中は押せず完了後に押せる／
  全体完了181秒。`npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-04 【重大】AI遺影で別人が生成される原因を特定し、16:9のAI生成を廃止
- 症状(実データ・北川様): 背景を指定して加工したところ、出力(iei-monitor-16x9.jpg)が
  **まったくの別人**になっていた。実際の方は生え際が後退し髪が薄いが、出力は髪がふさふさで
  顔立ちも若く別人。花束・服・姿勢まで描き直され、スタジオ写真のような別画像になっていた。
- 切り分け（実際にAPIを叩いて10通り検証。すべて縦の生成では本人のまま再現できず）:
  A 元のまま+背景青／B 顔中心に切って+背景青／C 背景そのまま／D 肖像生成／E お任せ／
  F 高度+正面+笑顔／G 高度+手を下ろす／H 服の見本つき(image[])／
  I〜K AI結果をさらに3回AIへ通す（劣化の確認）→ **いずれも本人のまま**。
- **原因（再現に成功）: 16:9(wide)の工程**。縦のAI結果を横長キャンバスへ置き、
  左右だけを生成させるために mask を渡していたが、**gpt-image 系は mask を渡しても
  画像全体を描き直す**（DALL·E 2 のように未指定領域を貼り戻さない）。
  結果、守っていたはずの中央の人物まで作り替えられ、別人が出力されていた。
  同じ手順を再現したところ、本人の縦画像から**髪がふさふさの別人**が生成され、症状と一致した。
  7/30の木村様の件で「16:9のほうが髪が整って見える」と感じたのも、同じ現象の軽い版だった。
- **対応: 16:9でAIに人物を描かせない。** 縦のAI結果をそのまま中央に置き、左右は同じ写真由来の
  背景をぼかして埋める局所合成(exportMonitor169FromBase)に一本化した。
  - 人物のピクセルは縦の結果のまま。別人化は構造的に起こらない。
  - 副次効果: AI呼び出しが2回→1回。**所要 約3.5分→約2分、費用 約50円→約29円**。
- 再発防止:
  - 生成条件（モード/背景/服装/見本の有無/体勢/表情/入力サイズ）をサーバーログに記録。
    今回、条件が分からず再現に時間を要したため。写真そのものは残さない。
  - コードに「gpt-image は mask を渡しても全体を描き直す。人物をAIに触らせる工程を増やさない」
    と理由つきで明記。
- 併せてご要望対応: 背景に「**お任せ**」を追加（元写真の人物・服装・明るさに合う背景をAIが選ぶ）。
- 検証（ポートを空けてから実施）: 背景に「お任せ」が出る／実写真でAI呼び出しが1回だけ(121秒)／
  4サイズDLの16:9が、縦のAI結果と同一人物（薄い生え際もそのまま）であることを目視確認。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-04 本番反映（16:9のAI生成廃止・背景お任せ）
- push 済み(03320f5)。本番 kawaguchi-fuhou.vercel.app で確認（2項目 ✓）:
  背景に「お任せ」が出る／実写真で生成し、AI呼び出しが1回だけ（＝16:9のAI生成が走らない）。
  16:9の中身はローカルの本番ビルドで、縦のAI結果と同一人物であることを目視確認済み。

## 2026-08-04 16:9を「左右まで背景が続く1枚」に作り直し／持ち物を外す指定を追加
### 16:9 の作り方（ご指摘: 左右が無くて違和感）
- 前回、別人化を止めるため16:9のAI生成を廃止したが、左右を「写真全体をぼかして敷く」実装のままに
  していたため、左右に人物の影がぼんやり出て、モニターに映すと不自然だった。作り直した。
- 試した順と結果（すべて実データで目視確認）:
  1. 写真全体をぼかして敷く（従来）→ 左右に人物の影が出る。不採用。
  2. 写真の左右端の帯を引き伸ばす → 端に花束や服がかかっていると横に伸びて汚れる。不採用。
  3. 選んだ背景の色で塗る → AIが実際に描いた背景と色がずれ、縦の継ぎ目が線として見える。単体では不足。
  4. **採用: 縦写真の上部左右の隅から実際の背景色を読み取り、その色の淡いグラデーションで全面を塗る。**
     そのうえで縦写真を中央に置き、左右の端を写真幅の12%かけて透過させてなじませる。
     遺影の構図では上の隅はほぼ確実に背景なので、人物の色を拾わない。読み取れない場合は
     選んだ背景の色にフォールバック。
- 結果: 左右まで背景が続く1枚の写真になり、継ぎ目もぼかしの塊も見えない。人物は縦の結果のままなので
  別人化は起きない。
- 各サイズの派生元について: ご提案は「16:9を作ってから各サイズに合わせる」だったが、
  四つ切りは3000x3600、16:9は1920x1080で、16:9から切り出すと**印刷用が3倍以上の引き伸ばし**になる。
  そのため、**印刷用（基準・手札・四つ切り）は縦の原寸から、16:9はその縦写真から組み立てる**形にした。
  見えている人物・背景はどのサイズでも同一で、16:9だけ左右が足されている状態。

### 「持っているものを外して腕を下ろす」チェックを追加
- 「AI詳細」の体勢・向きの下に新設。花束・賞状・杯・マイク・書類・他の方の手などを消し、
  そのうえで腕を下ろす。消した裏側の服・体・背景を自然に補い、影や輪郭の跡を残さないよう指示。
  顔の向き・表情・髪型・髪の量・服装・顔のサイズと位置は変えない、も明記。
- 既存の「手・腕を下ろす」は残し、持ち物ごと消したいときはこちらを使う（両方入れた場合は新しい方が優先）。
- 検証: チェックが表示され選択できる／実写真で生成しAI呼び出しは1回（121秒）／
  4サイズDLの16:9が左右まで背景の続く1枚になっていることを目視確認。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-04 本番反映（16:9の作り直し・持ち物を外す指定）
- push 済み(4235e32)。本番 kawaguchi-fuhou.vercel.app で確認（4項目 ✓）:
  「持っているものを外して腕を下ろす」が表示される／4サイズDLが動く／
  16:9が1920x1080で左右まで背景の続く1枚になっている（継ぎ目・ぼかしの塊なし・人物は本人のまま）／
  四つ切りが3000x3600のまま（16:9からの引き伸ばしになっていない）。

## 2026-08-04 【設計変更】背景と人物を分けて作り、重ねる方式へ（ご提案の採用）
- ご提案: 「16:9の背景を用意し、人物は人物で加工し、あとで重ねる」。これが正しい設計。
  現行の「1枚の写真の中で背景ごと作らせる」方式では、(1)変更範囲が大きく別人化しやすい、
  (2)縦写真を16:9に置くと背景の継ぎ目が出る、の両方が構造的に避けられなかった。
- 検討した切り抜き手段と結果:
  1. 自前の色キー（縁からの塗りつぶし）→ 明るい縞シャツと花束が背景色に近く、
     服や花が穴だらけになった。実データで確認、不採用。
  2. ブラウザ内のセグメンテーション（MediaPipe等）→ 髪の輪郭が粗く、遺影には不足。追加容量も大きい。
  3. **gpt-image の背景透過（background=transparent）→ 採用。**
     ※ gpt-image-2 は非対応（"Transparent background is not supported for this model."）。
       透過は **gpt-image-1** のみ。人物レイヤーだけこのモデルを使う。
- 新しい流れ:
  1. **人物レイヤー**: AIには背景の指示を一切せず、人物（と持ち物）だけを透過PNGで作らせる。
     髪型維持・服の着せ替え・手を下ろす・持ち物除去・表情はここで行う。
     背景を触らせないぶん変更範囲が小さく、別人化しにくい。
  2. **背景レイヤー**: 選んだ背景を、各サイズごとにこちらで全面に描く（単色・グラデは完全に自前）。
  3. **重ねる**: 縦・手札・四つ切り・16:9 のいずれも「背景1枚＋人物」を重ねて作る。
     どのサイズでも背景は連続した1枚なので、継ぎ目・色ずれ・ぼかしの塊が生じない。
     16:9は人物を高さいっぱいに置くため、持ち物が下で切れていても画面下に自然に収まる。
- 効果（実データ・北川様で確認）:
  - 16:9が「左右まで背景が続く1枚の写真」になった。継ぎ目・ぼかしの塊なし。
  - AI呼び出しは1回のまま。**所要 61秒**（従来3〜4分 → 2分 → 1分）。
  - 費用は gpt-image-1 のため単価は上がるが呼び出しは1回で、従来の2回分とほぼ同等。
- 検証: 実写真でAI呼び出し1回・61秒／4サイズDLの16:9が左右まで背景の続く1枚／
  四つ切り3000x3600のまま／人物は本人のまま。`npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-04 人物の輪郭に出る白フチ（ハロ）を除去
- 本番反映後の確認で、淡い背景に重ねると**人物の周りが白く光って見えた**。
- 原因: AIが返す透過PNGは、髪や肩の縁が広めの半透明（全画素の約19%が半透明）で、
  その半透明画素に元の白い背景が混ざっている。淡い背景に重ねると白フチとして目立つ。
- 対応: `removeWhiteFringe()` を追加。半透明画素は「人物の色×α ＋ 白×(1-α)」なので、
  白の分を差し引いて人物の色だけに戻す（αは触らないので毛先の柔らかさは残る）。
  縁を削る方法（最小値フィルタ）も試したが、内部まで半透明のため画像全体が痩せて破綻した。不採用。
- 検証: 実写真で生成（80秒）。白フチが消え、淡いブルーの背景に人物がきれいに乗ることを目視確認。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-04 本番反映（重ね合わせ方式・白フチ除去）
- push 済み(f8f93f0)。本番 kawaguchi-fuhou.vercel.app で実写真により確認:
  生成はAI呼び出し1回・**64秒**／4サイズDL可／16:9は1920x1080で
  **左右まで背景の続く1枚**（継ぎ目・ぼかしの塊・白フチなし）／四つ切りは3000x3600のまま／
  人物は本人のまま。
- 現状の作り: 人物レイヤー（透過・gpt-image-1）＋ 背景レイヤー（各サイズごとに全面描画）を重ねる。

## 2026-08-04 【修正】人物の切り出しを「緑背景＋クロマキー」に変更（gpt-image-1 の別人化を解消）
- ご指摘(実データ・北川様): 顔が若返って別人になり、服も色が変わって別の服に見える。
- 原因: 人物レイヤーに **gpt-image-1** を使っていたため。APIの背景透過に対応しているのが
  このモデルだけだったが、gpt-image-1 は顔を滑らかに若返らせ、服の色（青灰の縞→ベージュ）まで
  作り変えてしまう。gpt-image-2 なら本人らしさも服の色も保てることは、以前の検証で確認済みだった。
- 対応: **gpt-image-2 に「一色の明るい緑（0,177,64）の無地背景」で出させ、ブラウザ側で緑を抜く**
  （撮影のグリーンバックと同じ考え方）。
  - 緑は肌・髪・衣服のどの色とも離れているため、明るい縞シャツや淡い花束でも穴が開かない
    （白背景で色キーを試したときは服と花が抜けてしまった）。
  - `keyOutGreenScreen()`: 緑の強さ(G - max(R,B)) で 60以上→背景、18以下→人物、間は半透明。
    併せて輪郭に乗った緑かぶりを抑える。
  - route から background=transparent と gpt-image-1 を撤去し、「二度と使わない」理由をコードに明記。
- 結果（実データで確認）: 顔のシワ・薄い髪・青灰色の縞シャツ・青いボタンが元のまま。
  背景は連続した1枚。緑の残り・白フチ・継ぎ目なし。生成142秒。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-04 本番反映（緑背景＋クロマキー）
- push 済み(7a280da)。本番 kawaguchi-fuhou.vercel.app で実写真により確認:
  生成147秒／4サイズDL可／16:9は左右まで背景の続く1枚／
  **顔のシワ・薄い髪・青灰色の縞シャツ・青いボタンが元のまま**（若返り・服の色変わりなし）／
  緑の残り・白フチ・継ぎ目なし。

## 2026-08-04 AI加工を「1回の生成で背景ごと仕上げる」方式へ差し戻し（ご指示）
- ご指示: AI加工の出来が悪いので、別人化の指摘をいただいた時点の方式まで戻す。
  背景「お任せ」など、AI加工以外の追加分は残す。
- 戻した内容（人物レイヤー方式を撤去）:
  - 緑背景での生成とブラウザ側のクロマキー（keyOutGreenScreen）を削除。
  - route の layer=person、buildPersonLayerPrompt、client の personLayer 引数を削除。
  - 16:9 の「背景＋人物を重ねる」分岐と、そのための引数を削除。
  - AI生成は従来どおり buildAiPrompt による1回の生成（背景もAIが描く）に戻した。
- 残したもの（ご指示どおり）:
  - 背景「お任せ」／「持っているものを外して腕を下ろす」チェック／生成中の経過時間表示／
    髪型と髪の量を保つ指示／16:9のAI生成を行わない方針（別人化の原因だったため）／
    生成条件のサーバーログ。
- 16:9は、この1枚の写真を中央に置き、写真上部の隅から読み取った背景色で左右を埋める形に戻る。
- 検証: 背景「お任せ」と持ち物チェックが残っていること／AI呼び出し1回（145秒）／4サイズDL可／
  実写真で顔・髪・服装が元のままであることを目視確認。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-04 16:9のAI生成を「選べる設定」として復帰（既定はオフ）
- ご指示により16:9のAI生成を戻したうえで、実データ（北川様）で再確認したところ、
  **やはり別人が出た**。しかも重要な事実が判明した:
  **16:9をAIで作ると、他のサイズ（基準・手札・四つ切り）もその16:9から切り出される**ため、
  4サイズすべてが別人になる（exportFromWideMasterByKind の仕様）。
  同じ生成の縦単体は本人のままでも、書き出しは16:9由来になるため意味がない。
- そこで、戻したうえで**画面の設定（チェック）で選べる**形にした。**既定はオフ**。
  - オフ: 16:9は縦の結果から組み立てる（人物はAIに触らせない）。AI呼び出しは1回。
  - オン: 16:9をAIで生成し、全サイズがそこから派生する。左右は自然に広がるが、
    別人になることがある旨を、チェックの説明文に赤字で明記した。
- 検証: チェックが表示される／既定はオフ／オフのときAI呼び出しは1回で16:9にAIを使わない、を確認。
  `npm run lint` 0件・`tsc --noEmit` 0件・`next build` 成功。

## 2026-08-05 本番反映（AI加工の差し戻し・16:9のAI生成を設定化）
- push 済み(ae56be6)。本番 kawaguchi-fuhou.vercel.app で確認（4項目 ✓）:
  背景「お任せ」あり／「16:9もAIで作る」が表示され**既定はオフ**／
  「持っているものを外して腕を下ろす」あり。
- 現在の既定の流れ: AI生成は1回（背景ごと仕上げる従来方式）。16:9は縦の結果から組み立てる。

## 2026-08-06 個別依頼: 酒井様のAI遺影を部分補正（ほおの赤み／唇）
- ご依頼: ほおがアザのような色になっているので肌色に合わせる。唇を少し赤めに。同じフォルダへ保存。
- 対象: F:\onedrive\...\20260808 酒井\AI2\加工\iei-monitor-16x9.jpg (1920x1080)
- 方針: AIで作り直すと顔ごと変わる恐れがあるため、**AIは使わず画素単位の部分補正**で行った。
  補正した範囲以外は1画素も変えていない。
  - ほお: 中心(1088,466)・半径62x46の楕円をなめらかな重みで指定。
    明るさを合わせると顔が暗くなるため、**赤みの差だけ**を直した
    （患部の赤み52 → 周りの肌37に合わせて、超過分を引く）。質感・シワはそのまま残る。
  - 唇: 口元だけを対象に、色で唇を選択（赤みが強く、かつ青紫寄り＝歯や肌と区別できる）。
    3,274画素に赤を加算（R+20 / G-8 / B-4、境目が出ないようぼかして適用）。
- 出力: 同じフォルダに **iei-monitor-16x9-補正.jpg**（元ファイルは残した）。
- 目視確認: ほおの赤い斑が消え、周囲の肌となじむ／唇が自然に赤くなり歯は変化なし／
  顔・髪・服・背景は元のまま。

## 2026-08-06 酒井様のAI遺影を AI で部分補正（やり直し）
- 前回の画素単位の補正では「全然変わっていない」とのご指摘。AI加工でやり直した。
- 進め方（顔全体をAIに渡すと髪や目まで描き直されるため、範囲を段階的に絞った）:
  1. 顔まわり(800,190)520x520 → ほお・唇は直ったが、髪の輪郭まで描き直された。不採用。
  2. (880,360)330x330 → 目の下端(y≈415)を含んでしまい、目も再描画された。不採用。
  3. **(900,425)300x300 → 採用**。目より下だけ。ほおの斑と口元が入る。
- 手順: 該当範囲を1024x1024に拡大してAI(gpt-image-2)へ→戻して元寸に縮小→
  縁38pxをなめらかに透過させて元画像へ合成。**変わったのは x909-1192 / y438-718 のみ**
  （差分を数えて確認）。目・眉・髪・服・背景は1画素も変わっていない。
- 結果: ほおの赤紫の斑が消えて周りの肌となじみ、唇に落ち着いた赤の口紅が入った。歯は白いまま。
- 納品: 同フォルダに **iei-monitor-16x9-補正.jpg**（元ファイルは残置）。
  併せて確認用に「比較_顔」「比較_口元」の左右比較画像も保存した。

## 2026-08-07 個別依頼: 小林様の証明写真をスーツに着せ替えてAI遺影に加工
- 元: 20260809 小林\SKM_C251i26080714420.jpg（証明写真のスキャン。青背景・黒い上着・顔の周りに余白なし）
- 手順:
  1. スキャンの白フチと角の丸みを外す（(28,26)から922x1026を切り出し）。
  2. 遺影の構図にするため、1024x1536のキャンバスへ人物を上寄せで配置し、
     下側に肩・胸元を描き足す余白を作った（元写真は首から下がほとんど無いため）。
  3. gpt-image-2 の edits に、この画像＋**スーツの見本画像**(public/iei-clothing/male_suit.webp)を渡し、
     アプリと同じ buildAiPrompt（高度AI補正／服装=スーツ／背景=お任せ／髪型・本人らしさの維持）で生成。
     加えて「青背景と黒い上着は除く」「肩から胸元を自然に描き足す」「眼鏡は元の形・色・位置のまま」
     「スキャンの紙フチ・角の丸みを残さない」を指示。
  4. 4サイズを書き出し（基準3:4=1800x2400／手札7:10=1051x1500／四つ切り5:6=3000x3600／
     モニター16:9=1920x1080）。16:9は写真の上の隅から背景色を読み、全面に敷いて人物を重ねた。
- 結果: 濃紺のスーツ・白シャツ・濃紺のネクタイに着せ替わり、背景は淡い上品なグレー。
  眼鏡の細いフレーム、白髪まじりの髪、目・鼻・口はご本人のまま。
- 納品: 20260809 小林\**AI加工**\ に 0_比較 / 1_基準写真 / 2_手札 / 3_四つ切り / 4_モニター を保存。

## 2026-08-10 個別依頼: 斉藤様のAI遺影を2パターン×4サイズで納品
- ご依頼: 背景「ピンクのグラデーション」と「（原稿は葉桜気味なので）もっと桜らしい背景」の2種類。
  それぞれ4サイズを 20260813 斉藤\加工 に出力。
- 元: S__113328139.jpg（1108x1477。桜の下で撮った全身寄りの写真）。
  加工フォルダにあった小さい切り出しではなく、**原本から切り出して**画質を確保した。
  切り出しは (405,505) 330x495 の2:3。手に持ったマスク・イヤホンが入らない高さで切った。
- 生成: gpt-image-2 の edits を2回。アプリと同じ buildAiPrompt（高度AI補正／服装そのまま／
  髪型・本人らしさの維持）に、以下を追加。
  - 共通: 柵・川・地面・山・空を消す／マスクの紐とイヤホンのコード・持ち物を消す／
    濃いグレーのニットコートは色柄襟元をそのまま。
  - ピンク: 淡いピンクの縦グラデーション、無地。
  - 桜: 満開の桜が画面いっぱい、やわらかくぼかす。葉桜にしない（緑の葉・枯れ枝を目立たせない）。
- 4サイズの書き出し: 基準3:4=1800x2400／手札7:10=1051x1500／四つ切り5:6=3000x3600／
  モニター16:9=1920x1080。
  - 16:9の背景は、桜のような情景では**人物が写っていない上端の帯を引き伸ばして**敷いた
    （写真全体をぼかすと人物の影が出るため）。単色・グラデーションでは上の左右隅の色から縦グラデを作る。
  - ※ 最初、グラデーションの下端の色を画像下部から拾ったため、服の色（濃いグレー）を拾って
    背景が下だけ黒くなった。上の隅の色だけを使い、下はその色を少し沈める方式に直した。
- 納品: 加工フォルダに ピンク_1〜4 / 桜_1〜4 の計8点。

## 2026-08-10 斉藤様・桜の背景の作り直し → OpenAIのクレジット切れで保留
- ご指摘: 桜の背景の左側がぼけている。ぼかさないでほしい。
- 原因: AIが被写界深度のある写真として描いたため、左側が前ぼけになっていた（こちらの処理ではない）。
- 対応予定の指示（作成済み・すぐ流せる状態）:
  「背景の桜は画面の左右どちらも同じようにはっきり見えるように」「片側だけをぼかす・全体をぼかす・
  手前だけにピントを合わせる表現はしない」「一輪ずつの花びらの形が分かる程度に左端から右端まで均一に」
- **実行できず**: OpenAI が 429「You have no credits remaining.」を返した（APIキー自体は有効・疎通200）。
  クレジットを追加していただき次第、上記で作り直す。

## 2026-08-10 斉藤様・桜の背景を作り直し（左のぼけを解消）
- クレジット追加後に実行。桜の4点を差し替え（ピンクの4点はそのまま）。
- 生成の指示に追加: 「背景の桜は画面の左右どちらも同じようにはっきり」「片側だけをぼかす・全体をぼかす・
  手前だけにピントを合わせる表現はしない」「一輪ずつの花びらの形が分かる程度に左端から右端まで均一に」
  → 左の前ぼけが消え、左右均一に咲いた背景になった。
- 16:9の左右の作り方で3回やり直した（いずれも実物を見て判断）:
  1. 上端の帯を引き伸ばす → 左右だけ花が大きくなり不自然。不採用。
  2. 帯を原寸のまま並べる → 帯に**髪が写り込んで**右側に黒い筋が出た。不採用。
  3. 人物が入らない左上の角をタイル状に敷く → 鏡合わせの模様がはっきり出た。不採用。
  4. **採用: 「桜だけ」の16:9背景をAIで別途生成し、その上に写真を重ねる。**
     人物の写り込みも模様の繰り返しも起きない。桜どうしをなじませるため、
     重ねる写真の左右13%を透過させた。
- 検証: 左右とも均一にくっきりした桜／髪の写り込みなし／鏡合わせの模様なし／人物は本人のまま。

## 2026-08-11 斉藤様・背景3案目「ピンク＋うっすら雲柄」を追加
- ご要望: ピンクの無地が単調なので、ピンクのグラデーションにうっすら雲柄を重ねた案も。
- 指示: 上を明るく下へ少し濃くなる縦グラデーション＋輪郭のない霞のような淡い濃淡。
  白っぽくごく控えめにし、はっきりした形の雲・青空・光線は入れない。人物より目立たせない。
- 16:9は桜と同じ作り方（同じ背景を「背景だけ」で別途生成し、その上に写真を重ねる。左右13%を透過）。
- 納品: 加工フォルダに **ピンク雲_1〜4** を追加。既存のピンク4点・桜4点はそのまま。
  これで3案×4サイズ＝12点。

## 2026-08-12 事務: スマート葬儀の2026年7月分 請求書をPDF出力（69件）
- ご依頼: 2026/07/01〜07/31 の請求書を通常印刷でPDF化し、
  F:\事務処理用_AI\末締め処理\202607\請求書 へ。印刷内容に担当者名を追記。出力後に供花と葬儀で分ける。
- 進め方（既存の tmp/smart/35-login-list.mjs のログイン処理を再利用。閲覧のみでデータは変更していない）:
  - 一覧の絞り込みは請求日のパラメータ `q[invoiced_on_gteq]` / `q[invoiced_on_lteq]` を使用（全2ページ・69件）。
  - 印刷用の帳票は `/users/customer_managements/invoices/{id}`（画面上部に「印刷」「編集」のある画面）。
    Playwright で開き、(1)画面用の「印刷」「編集」ボタンを非表示、(2)請求書番号・請求日の下に
    「担当者 ○○」を追記、(3)印刷用CSSを適用して A4（上下10mm・左右8mm）でPDF保存。
  - 担当者名: 請求書の「計上担当者」欄は**7月分は全件未入力**だったため、
    ご相談のうえ一覧の「最終更新者」の氏名（例: 松澤覚）を使用。日時は除いて氏名のみ。
  - ファイル名: `請求日_請求書番号_顧客名.pdf`（例: 20260701_2087600_川本 好映.pdf）。
- 結果: 69件すべて出力（失敗0）。件名で振り分け:
  **葬儀 16件 / 供花 53件**（「供花・供物・献花」を含む件名を供花とした）。
  `請求書\葬儀\` と `請求書\供花\` に格納。合計7.1MB。

## 2026-08-12 【修正】担当者名の列を取り違えていた
- 症状（ご指摘）: PDFの担当者欄が「0円 管理」になっていた。
- 原因: 請求書一覧の列を1つずらして読んでいた。**最終更新者は td[9]** なのに td[10]（伝票(入金)欄）を使っていた。
  ヘッダーは15列だが、行の td は先頭の「一括ロック」が無く1つ詰まっている。
  ヘッダーの並びだけを見て添字を決めたのが誤り。実際の行を添字付きで出力して確認すべきだった。
- 対応: td[9] から「氏名 2026/07/27 (月) 14:13」の氏名部分だけを取り出すように修正し、全69件を作り直した。
  例: 松澤覚 / 石川健太。
- 途中経過: 3件はPDFを開いたままで上書きできず（EBUSY）。一時ファイルに書いておき、
  閉じていただいたあとに置き換えて完了。
- 最終状態: 葬儀16件 / 供花53件（計69件）。一時ファイルの残りなし。
  再実行用スクリプトは tmp/smart/81-print2.mjs（一覧の絞り込み〜PDF出力〜振り分けまで）と
  tmp/smart/79-split.mjs（振り分けのみ）を残した。
