...
## フェーズ2: 弔意・祭壇・メッセージ
- ⏸ 2-10 メディアパイプライン（画像素材は最後にまとめて）
- ✅ 2-11 オンライン祭壇（入場/AltarView合成/焼香/記帳/アルバム/喪主挨拶）※実PNG素材は最後
- ✅ 2-12 バーチャル参拝 `/worship`（宗派別出し分け・匿名可）
- ✅ 2-13 お悔やみメッセージ `/message`（承認制=既定pending）

## 追加実装（実調査で判明・原典準拠）
- ✅ 管理画面: ダッシュボード/葬儀一覧/**作成ウィザード5ステップ**/設定/各種一覧（紫テーマ）
- ✅ 供花注文 `/flower`（札名・旧字体・特商法同意・受付終了判定）
- ✅ 香典決済 `/koden`（金額サーバ再検証・手数料負担選択・表書き宗派出し分け）＋ Stripe Webhook骨組み
- ✅ 香典帳 `/admin/koden` ＋ **CSVエクスポート**（半返し自動計算・Excel BOM）
- ✅ RSVP `/rsvp`（WEB出欠・差別化機能）
- ✅ GitHub接続・push（kawaguchi_fuhou, 機密除外済み）
