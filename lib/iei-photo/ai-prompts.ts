...
/**
 * 「胸元まで写す（引きの構図）」を選んだときの追加指示。
 * gpt-image は余白を足した入力を渡しても顔に寄せて描き直す癖があり、
 * 「手・腕を下ろす」系の指示にも「顔を大きく」と書いてあるため、最後に置いて上書きする。
 */
export const IEI_PHOTO_BUST_FRAMING_PROMPT =
  "構図についての最終指示（これより前の「顔を大きく」「頭から胸元を画面中心に大きく」という指示より優先）: " +
  "顔のアップにはせず、頭の上に画面の高さの1割ほど余白を取り、頭のてっぺんから帯の上（みぞおち）までが入る、" +
  "胸元から上のバストアップ構図にしてください。顔の高さは画面の高さの3分の1程度、両肩の幅は画面の幅の6〜7割程度とし、" +
  "肩と胸元、着物や上着の襟合わせがはっきり見えるようにしてください。" +
  "人物を小さくするために足した部分は、元の服装と同じ服・同じ背景が自然に続いているように描き足してください。" +
  "顔立ち、髪型、表情、本人らしさは変えないでください。";
