...
export const IEI_PHOTO_HAIR_PROMPT =
  "髪は生え際、分け目、毛の流れ、毛先、後れ毛を元写真のとおりに保ってください。白髪、細い髪、毛量の少ない部分、地肌が見えている部分も、元写真のまま1本ずつの毛の質感を残してください。髪の輪郭をなめらかに塗りつぶす、髪の量を増やす、減らす、頭の形やシルエットを整形する、生え際の位置を変えることは避けてください。髪と背景の境界では、細い毛先が背景に自然になじむようにし、光の輪、白い縁取り、黒い縁取り、貼り付けたような境目、不自然なぼかしを作らないでください。";
